...
2. **Module-key taxonomies don't match.** The nav trees gate items per SCREEN (`NAV_MODULE_KEYS`: `timetable`, `exams`, `marks`, `students`, …, 19 keys), transcribed from DESIGN-SYSTEM §3.2 in F-ID-03 Part 3. The plans engine (F-CM-06 Part 1, `20260917010300_plans_and_notifications.sql`) gates a plan per BUNDLE (`plan_modules.module`: `academics`, `attendance`, `lessons`, `messaging`, `resources`, `reports`, `print`, `hiring`, `cover`, `analytics`, `marketplace_school_funded`, `custom_labels`, `priority_support`). Only 7 of the 19 nav keys have any real plan-module equivalent at all; wiring the shell to `filterNav`'s `entitledModules` naively (treat "no matching plan module" as "not entitled") would have hidden most of the nav for every real workspace, which is a functional regression far worse than the drift being fixed.
...
2. **Module taxonomy — a documented, one-directional mapping (`packages/domain/src/nav/entitlements.ts`), never a blanket gate.** `NAV_MODULE_TO_PLAN_MODULE` names only the 7 nav keys that have a real `plan_modules` equivalent today (`attendance`→`attendance`; `timetable`/`students`/`exams`/`marks`/`assignments`/`curriculum`→`academics`; `lessons`→`lessons`; `resources`/`library`→`resources`; `reports`→`reports`; `print`→`print`; `messages`→`messaging`; `hiring`→`hiring`; `cover`→`cover`; `marketplace`→`marketplace_school_funded`). `buildEntitledNavModules(planModules)` (using the plans engine's own `hasModule`, not a hand-rolled `.includes()`) includes every UNMAPPED nav key unconditionally — `staff`, `billing`, `ai` have no plan gate yet, so they stay visible rather than silently disappearing because nobody has reconciled the two taxonomies. The school shell calls `getWorkspacePlan` + `listEnabledModules` (both already in `packages/db`, F-CM-06 Parts 1-3) and feeds the result through this function; a plan-lookup failure degrades to "every unmapped key visible, every mapped key gated off" rather than an empty nav. **Full reconciliation of the two taxonomies is out of scope for this Part** (a comment to this effect already existed in `NAV_MODULE_KEYS` before this PR) and is carried forward as a note in `DESIGN-SYSTEM.md` §3.2.
3. **`coverage.sql` — plain SQL for the DB-catalog half; a Node script for the file-presence half; scoped to `workspace_id` tables, not narrowed to RLS-off alone.** `psql`'s `db` job connects to a Postgres **service container** that is a separate machine from the GitHub Actions runner; the runner has the repository checkout, the service container does not, so `pg_read_file` inside it — even as the superuser `PGUSER=postgres` this job already connects as — cannot see `supabase/tests/*.sql` no matter what role reads it. `supabase/tests/coverage.sql` therefore does only the half provable against the live catalog in one `psql` session: every `public` table with a `workspace_id` column has `pg_tables.rowsecurity = true`, via a plain `do $$ … raise exception … $$` block (not pgTAP — this file is a `db`-job step of its own, run with `ON_ERROR_STOP=1`, separately from the `pg_prove` step). The "named in a `supabase/tests/*.sql` file" half is `scripts/check-coverage-test-files.mjs`, wired into `CI / contracts` (a plain checkout, no DB needed): it derives "table has a `workspace_id` column" the same way `check-audit-catalog-parity.mjs` already derives audit facts — parsing `create table public.<name> (...)` blocks across `supabase/migrations/*.sql` with paren-depth tracking, not a live query — then checks each qualifying table name appears in `supabase/tests/*.sql` (excluding `coverage.sql` itself, which is infrastructure, not a behavioural test). `supabase/tests/13_rls_grants_invariants.sql` (next free number after `12`) separately asserts the broader "every table in `public`, tenant or not, has RLS enabled" and "anon has no USAGE on schema `app`" — it does **not** re-assert `12_function_grants_invariant.sql`'s function-allowlist checks, per this Part's brief.
4. **A tracked baseline, not a silent pass, for pre-existing coverage gaps.** Running the new script for the first time found five real tenant tables — `consent_records`, `legal_acceptances`, `email_log`, `file_access_log`, `subscription_events` — with a `workspace_id` column and zero mentions anywhere in `supabase/tests/`, predating this PR and each needing its own RLS test design (PDPA consent capture, notification delivery logs, file access logs, subscription lifecycle events — none of them this Part's scope). Writing a new required check that is red on `main` on day one gets the check disabled, not the gap fixed, so `check-coverage-test-files.mjs` carries an explicit `KNOWN_GAPS` allowlist naming exactly these five, each with which future feature owns writing its test, and asserts the allowlist itself can only shrink (a table named in a test file while still listed in `KNOWN_GAPS` is itself a failure, so nobody forgets to delete the line).

**Why:** two independently-tested, independently-shipped systems claiming to be "the nav" is the exact drift ARCHITECTURE and CLAUDE.md rule 1 exist to prevent — a route guard and a rendered nav item disagreeing about visibility is precisely prototype finding D6 (`module_*` toggles no navigation code read) reappearing at the shell level. Picking the already-tested, zero-UI-dependency copy as canonical (rather than picking `packages/ui`'s richer-looking one and porting its tests) means the school shell inherits `filterNav`'s existing pgTAP-adjacent unit coverage for free, and keeps the dependency direction the codebase already has (`ui` → `domain`, never the reverse). Gating nav visibility on a module key with no real plan-catalogue equivalent, silently, is worse than the duplication it would have replaced: it would have shipped a nav nobody could see modules in.

**Consequences:** any new nav item that needs a plan gate must add its `plan_modules` equivalent to `NAV_MODULE_TO_PLAN_MODULE` explicitly — an unmapped `module` key on a `NavItem` is a promise nobody is keeping yet. `packages/ui`'s `nav-config.ts` module docblock and this entry are the record of the "single nav system" shape; a future change to any of the five curated trees is a change to `packages/domain/src/nav/config.ts` only. `DESIGN-SYSTEM.md` §3.2 gets a short note pointing here. The `KNOWN_GAPS` allowlist in `scripts/check-coverage-test-files.mjs` is real, tracked technical debt — closing each entry is future work, not something this Part claims to have done.
