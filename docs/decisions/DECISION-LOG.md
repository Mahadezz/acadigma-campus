...
<!-- Numbering note: D-51/D-52/D-53 are reserved by open PRs #6, #12 and #13
     at the time this entry was written; this entry is D-54 to avoid a
     collision. If one of those PRs merges with a different number, do not
     renumber this entry — add a superseding note instead (numbers are never
     reused, per this file's header). -->

## D-54 — Deny function EXECUTE by default in `public`; CI mirrors Supabase's default privileges · ACCEPTED · 2026-09-24

**Context:** Live verification against the production Supabase project `kekfmibwjejdhxjkmezo` (all 7 main migrations applied) showed `has_function_privilege('anon', 'public.switch_workspace(uuid)', 'execute')` returning `true`, despite `20260917020300_tenancy_hardening.sql` §5 granting that function to `authenticated` only. The cause: Supabase provisions every project with `ALTER DEFAULT PRIVILEGES FOR ROLE postgres, supabase_admin IN SCHEMA public GRANT EXECUTE ON FUNCTIONS TO anon, authenticated, service_role`. Every migration's `revoke all on function ... from public` revokes the PUBLIC pseudo-role's grant, not the explicit per-role grant the default already handed to anon/authenticated at `CREATE FUNCTION` time — so the "revoke, then grant back the intended roles" pattern used throughout `identity_auth.sql` and `tenancy_hardening.sql` left the unintended roles granted anyway. Affected: `public.log_auth_event_service` (intended service_role-only — anon/authenticated could forge `account.registered` audit rows with an arbitrary user id, ip and user agent), `public.switch_workspace`, `public.list_my_workspaces`, `public.log_tenancy_context_rejected` (all intended authenticated-only). `log_tenancy_context_rejected` additionally has no `auth.uid() is null` guard, so this was live-exploitable: anon could write a `tenancy.context_rejected` audit row into any workspace with an attacker-chosen `attempted_workspace_id`. CI never caught this because its Postgres 17 service container has no such default, so `has_function_privilege('anon', ...)` pgTAP assertions passed for the wrong reason.
**Decision:** (1) `supabase/ci/bootstrap.sql` now runs the same `ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public GRANT EXECUTE ON FUNCTIONS TO anon, authenticated, service_role` Supabase runs on project creation, so CI's grant assertions are false exactly when they would be false live. (2) `20260924030000_revoke_default_function_grants.sql` revokes the mis-granted EXECUTE from the four functions above and, going forward, flips the default itself: `alter default privileges for role postgres in schema public revoke execute on functions from anon, authenticated` (service_role keeps it) — every new `public` function is deny-by-default and must grant EXECUTE explicitly, or it is uncallable rather than silently open. (3) `supabase/tests/12_function_grants_invariant.sql` asserts both the allowlists and the default-ACL shape directly, so a regression fails CI instead of needing platform verification to catch it. `log_tenancy_context_rejected`'s missing null-check is NOT fixed in this PR — its body is already being replaced by PR #12 (`fix/tenancy-review-followups`, `20260924020000_tenancy_tripwire_membership_status.sql`); adding the check here would conflict. The grant fix alone closes the PostgREST path; the follow-up (adding `if auth.uid() is null then raise exception ... errcode 42501` to match its siblings) belongs to PR #12 or a tiny PR immediately after both merge.
**Why:** a security boundary that depends on the platform's default privileges silently agreeing with the repo's intent, with nothing testing that they do, is not actually enforced — CLAUDE.md rule 1 ("the database and the server are the security boundary") requires the database layer to be verified, not assumed. Deny-by-default for `public` function EXECUTE makes "forgot to grant" fail loud (uncallable) instead of failing open (callable by everyone holding the publishable key).
**Consequences:** every future migration that adds a `public` function MUST grant EXECUTE explicitly to whichever of anon/authenticated it intends, or the function is uncallable from the client entirely (service_role is unaffected). `supabase/tests/README.md` documents the new test file.
