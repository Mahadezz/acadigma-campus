...
1. A teacher marks a 40-student section's attendance in **≤ 60 seconds** on a 360 px phone (measured by e2e + field test).
...
4. 10 schools onboarded; ≥ 70 % of their teachers active weekly.
...
| **R4 Native & automation**   | Everywhere               | Android (Capacitor) with push + QR scan + offline attendance; Windows (Tauri) with print agent; gate-scan attendance input; scheduled digests                                                                                                                                                  | Apps in Play Store / installer; local printing from the queue.                                                                   |
