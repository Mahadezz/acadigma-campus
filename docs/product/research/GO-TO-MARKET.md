...
- Schools that want **fee collection / accounting as the first module**. Campus v1 has an expense ledger and school billing, not a student fee-collection engine. Promising it is how you burn the first 10 references. (Route to the Acadigma Admin app on the roadmap; take a waitlist email.)
...
| ---------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sep–Oct**      | Annual exams looming; next year's budget being thought about; admission prep           | **Prime pilot window.** "Run your annual exam on Acadigma for free; if the প্রগতিপত্র comes out clean, start January on a paid plan."           |
| **Nov–mid Dec**  | Annual exams, results, admissions, chaos                                               | **Prove value, do not sell.** Be present. Generate their report cards. This is where references are earned.                                     |
| **Late Dec–Jan** | New academic year setup: new sections, promotions, new students, new teachers          | **Prime closing window.** Setup cost is lowest and switching cost is lowest exactly now. Target: convert every pilot to paid before 15 January. |
| **Feb–Apr**      | Term running; Ramadan/Eid disrupt (also: ad CPMs spike 2–3× at Eid — pause paid spend) | Land-and-expand inside existing schools; teacher-side growth; marketplace supply                                                                |
| **May–Aug**      | Half-yearly exams; quieter admin                                                       | Second pilot cohort; content and community; Tier B city expansion                                                                               |
...
| 3   | "Our teachers aren't tech-literate."                                         | "They already use Facebook and bKash. If a teacher can send a Messenger voice note, she can take হাজিরা. Watch —" [do the 60-second demo on your phone, then hand your phone to the oldest teacher present and let _her_ do it]. This single move closes more schools than any slide.                                                                                                                                                             |
...
| 12  | "Does it do fee collection?"                                                 | Tell the truth: "Not in this version — Campus does academics, attendance, exams, report cards, communication and staff. Fees/accounts are the Acadigma Admin app on our roadmap. If fees are your only problem, I'd rather you wait than be disappointed." Take their email for the waitlist. Honesty here buys the next three referrals.                                                                                                         |
...
| 5   | **Security one-pager**                                                                                    | PDF: RLS/tenant isolation, audit, PDPO 2025 alignment, pentest date | Before first paid school           |
...
**C. Teacher → their School (the pull-through).** A teacher on a personal workspace who invites her principal, resulting in a school that reaches paid: **৳2,000 bKash or 12 months of marketplace commission-free selling**, teacher's choice. This is the single most valuable referral type — it turns your free tier into a sales channel — so it's the only one that pays cash.
...
| **Commission**           | **25% of collected subscription revenue for the first 12 months**, then 10% recurring while the school stays paid. Marketplace GMV excluded.              |
| **Clawback**             | Full clawback if the school churns within 90 days — this is what stops overselling.                                                                       |
...
- **Never** put student names or marks in a WhatsApp message. Send a link into the parent portal instead. This is both a PDPO 2025 posture and a real safeguarding rule.

### 4.10 Print and offline

Cheap, local, and it reaches the schools with no Facebook page:

- **School stationery and guide-book vendors** — the shops that sell attendance registers (হাজিরা খাতা), report-card booklets and exam pads to every school in a thana. Put a Bengali card in every register they sell, with a QR to a free template pack. Pay per scanned lead (৳50–100) or per converted school (৳1,000). ৳3,000 of printing covers a thana.
...
- **Mechanic:** Every student record carries guardians with phone numbers. When a school publishes results or a notice, guardians without accounts receive an SMS/Messenger link to a **single-child, read-only view**; the page ends with "See attendance and marks any time — set a password" → account created → the parent now returns weekly. Parents who log in see a prompt to invite the other guardian.
...
- **Release:** **R3.** (Jobs in BD education groups get enormous organic reach — this loop is underrated.)

### 5.5 Share-to-WhatsApp of notices and report cards, with branding

- **Mechanic:** Every notice, প্রগতিপত্র and নম্বরপত্র has a "শেয়ার করুন" action producing a **link with an OG preview card** carrying the _school's_ logo and name and a small "Acadigma Campus" line. Parents forward these to family; other parents at other schools see them. **Never share a PDF with marks into a chat — always a permissioned link** (safeguarding + PDPO 2025).
- **Metric:** shares per school per week; click-through on shared links; signups attributed to `?ref=share`.
- **Where:** report PDF/share service, notices, OG image generation.
- **Release:** **R1** (notices), **R1/R2** (report cards).

### 5.6 "Powered by Acadigma" on Free-plan PDFs

- **Mechanic:** Free-plan PDFs (প্রগতিপত্র, নম্বরপত্র, registers, ID cards) carry a small, tasteful footer: `Acadigma Campus দিয়ে তৈরি · acadigma.com` with a QR. Removed on Starter and above — which is also a clean, non-punitive upgrade lever. **Design rule: it must never make the school look cheap.** Small, grey, bottom-right, outside the signature area.
- **Metric:** Free→Starter conversions citing branding; scans of the PDF QR.
- **Where:** PDF renderer, plan entitlements.
- **Release:** **R1.**

### 5.7 Referral codes with credits

- **Mechanic:** Implements §3.5. Every user has a code; every workspace has a code. Rewards are AI credits (teacher↔teacher), subscription months (school↔school), or cash (teacher→school). Rewards vest on the referred party's _activity_, not signup. A visible referral panel shows status of each referral.
- **Metric:** _k-factor_ = referred signups ÷ active referrers. Target k ≥ 0.15 by month 6 (it will not be ≥1 — that's fine; referral is an accelerant, not the engine).
- **Where:** `/personal` and school settings; credit ledger integration; audit entries for every grant.
- **Release:** **R2** (credits version), **R3** (cash + subscription version, needs billing).
...
**Stay per-school with student/teacher caps.** Per-student pricing is transparently "a tax on growth" to a school owner and invites haggling over headcount every term; caps deliver the same revenue expansion invisibly (a school that grows past 600 students upgrades to Pro because it must, not because you asked). **Exception:** Enterprise, where per-student or per-campus negotiated pricing is normal and expected above ~2,500 students.
...
- **What they get:** **Starter free for 12 months**, renewable on reapplication, with a "Supported by Acadigma" note on their public profile (their choice).
...
| ------------------------------------------ | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Attendance completeness falling**        | < 60% for 5 school days                 | Highest-signal leading indicator. WhatsApp the champion _that day_: "I noticed Class 6 হাজিরা hasn't been marked since Sunday — anything broken?"     |
...
The **Personal Data Protection Ordinance 2025** (promulgated 6 November 2025) requires parental consent for processing children's data, prohibits tracking, profiling and targeted advertising directed at children, and carries criminal penalties (reported as up to 5–7 years' imprisonment and fines up to ৳20 lakh for serious violations, including misuse of children's data). Your policy and your product must both reflect this — and the product decisions already point the right way (data minimisation, private buckets, audit trails, no student PII in AI prompts beyond first name + grade).
...
17. **Personal Data Protection Ordinance 2025, Bangladesh** (promulgated 6 November 2025; parental consent for children's data; prohibition on tracking, profiling and targeted advertising directed at children; criminal penalties reported up to 5–7 years and fines up to ৳20 lakh). https://securiti.ai/bangladesh-personal-data-protection-act-overview/ · https://mahbub-law.com/key-highlights-of-the-personal-data-protection-ordinance-2025-for-businesses/
