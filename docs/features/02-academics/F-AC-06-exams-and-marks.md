...
**Enums**: `exam_kind`, `exam_type`, `exam_status`, `exam_subject_status`, `mark_status`, `result_scope`, `result_status`, `optional_rule`, `subject_kind` (`compulsory|optional_fourth`; declared alongside `subjects` in F-AC-01), `comment_source`.
...
7. **Exam-hall seating and invigilation.** `invigilator_id` and `room_id` exist on the paper; seat plans are out of scope for v1.
