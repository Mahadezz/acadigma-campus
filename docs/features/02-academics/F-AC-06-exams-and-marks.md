...
**Part 7 status (demo cut, D-306):** "Publish" on a `marks_locked` exam opens a sheet that explains the freeze and lets the admin withhold students, each with a reason; `public.publish_results` (owner/admin, `results.publish`) sets the reasons and moves the exam to `published`. Whichever path publishes (this or the plain status step), `app.tg_exams_publish_results` runs after D-304's `MARKS_INCOMPLETE` gate and refuses `NOT_COMPUTED` and `INCOMPLETE_PRESENT`, so nothing incomplete is ever published; then every result gets `published`, `published_at`, `published_by` and a `frozen_payload` built in SQL (`app.result_card_payload`: the report card's `ReportCardDto`, with a withheld student frozen as `withheld`, no GPA, grade or rank), with one `results.published` audit event (counts and the withheld ids). Every report card read uses the frozen payload when present, so renaming a subject or section afterwards changes nothing (AC16). Unpublish is the existing `published → marks_locked` step with a reason: results are hidden again, the payload is kept, `results.unpublished` records the reason; results cannot be recomputed while published. Parents read a result (and its lines) only when it is published, not withheld, they are an active parent member of the school and have an active `guardian_users` link to the child (AC15, pgTAP: not another family's child in the same section, not a withheld one, not before publishing or after unpublishing, not through a revoked link or a removed membership, not another school). `guardian_users` is created here in F-AC-02 §3's shape with no client write grant — **the invite flow that links a parent to a child is F-AC-02 Part 4, so until it lands no parent is linked in the product.** The family screen and "Download report card" are F-AC-10's (see its §11). Deviations: no `result.published` notification (F-ID-07 has no delivery yet; `TODO(D-306)` in the trigger), no republish notice, school branding not frozen (read at render), withholding chosen at publish only (release = unpublish and republish), no idempotency key or rate limit. See D-306.
