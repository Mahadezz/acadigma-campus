...
**Offline marks sync (owner decision, 2026-09-26; for when the offline queue lands in Part 4 — not built yet):** sync is **per cell**: the later edit wins, and the overwritten value is kept in the audit trail. This replaces §4.2's "same queue and idempotency contract as F-AC-03" for marks: no whole-paper conflict sheet.
