...
7. **Part 1 demo cut (D-202, PR #43).** Built: `holidays` (no `academic_year_id`, no scopes, no recurrence yet), `working_day_overrides`, `app.is_school_day(workspace_id, date)`, `app.school_days`, `app.school_day_count`, and a holidays list/add/remove screen at `/app/settings/calendar` (the settings home links to it; `/app/calendar` views are Part 3). Deviations: the functions are **SECURITY INVOKER**, not DEFINER, because they read only tables every staff member may read and the caller's RLS is the right filter. There is no `section_id` argument until `holiday_scopes` exists, which needs `sections`. `academic_year_id` arrives once `academic_years` (PR #37) is on main. Still open: the TS mirror with its SQL↔TS parity test, `GET /api/calendar/school-days`, and an override screen (the table and precedence are built and tested).
