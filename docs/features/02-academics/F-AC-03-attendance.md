...
| ------------------------------------------------------------- | ------------------------- | -------------------------------- | ----- | --------------------------------------------------------------------------------------- | ----- | ----------------- | -------- |
| Open the roll-call screen for a section                       | `attendance.take`         | yes                              | yes   | class teacher, or a teacher with a `section_subjects` row in that section (period mode) | no    | no                | no       |
| Save / update a session inside the edit window                | `attendance.write`        | yes                              | yes   | same as above                                                                           | no    | no                | no       |
...
A teacher may take attendance only for sections they are attached to. Admins may take it for any section (they cover). Staff are read-only for reporting. Parents read only through F-AC-10.
...
**`attendance_sessions`** — one per section per date (per period when the school uses period mode): `section_id`, `academic_year_id`, `date date`, `period_no int null` (null = daily roll-call), `timetable_slot_id uuid null`, `section_subject_id uuid null` (period mode), `taken_by uuid references workspace_members`, `taken_at timestamptz`, `status attendance_session_status` (`draft|submitted|locked`), `present_count int`, `absent_count int`, `late_count int`, `excused_count int`, `half_day_count int`, `expected_count int`, `source attendance_source` (`manual|scan_prefill|bulk|import`), `note text null`, `locked_at timestamptz null`, `edited_after_window bool default false`.
...
1. The screen opens with **every enrolled student pre-marked present** (default from `policy.default_status`, see §5.2) and a big header "38 present · 0 absent".
2. The list is 64 px rows: avatar, roll number, name. Tapping anywhere on a row **cycles** present → absent → late → present. The current state is shown as a large coloured pill on the **right edge** of the row, inside the thumb arc.
...
4. A sticky bottom bar shows the running counts and one primary **Save** button, full-width, 56 px tall, in the thumb zone. Secondary actions (Mark all present / Mark all absent / Copy yesterday) sit in the top bar, away from the thumb, so they cannot be hit by accident.
5. Save writes the session + all records in one server action.
   ...
   _Audit:_ `attendance.session.saved` with counts; `attendance.record.changed` (before/after) for every later edit, never for the first save (which is covered by the session event).
...
**5.2 Default status.** A session opens with every enrolled student at `policy.default_status` (`present`). This is a UI default only — the records are not written until Save, so an unsaved session is not a present-marking.

**5.3 Expected students for a session** = students with an `enrollments` row for that section where `status='active'`, `enrolled_on <= session.date` and (`ended_on is null` or `ended_on >= session.date`), and whose `students.status='active'` and `deleted_at is null`. A student transferred on 1 March is expected in the old section up to 28 Feb and the new one from 1 March — nothing else needs to change.
...
| `markAllPresent`                              | `MarkAllInput` {sectionId, date, status: 'present'                                                                                                        | 'absent'}                                                          | `AttendanceSession`                                                                                                           | same as save                         | key required  | 120/h |
...
1. **Given** a class teacher on a 360×800 phone with 40 students, **when** they open the roll-call screen, **then** all 40 are pre-marked present, the Save button is within the bottom 88 px, every status pill is right-aligned within the thumb arc, and no horizontal scrolling exists.
2. **Given** that screen, **when** the teacher taps 4 rows once each and Save, **then** exactly one `attendance_sessions` row and 40 `attendance_records` rows exist, with 36 present and 4 absent, and the whole interaction is completable in under 30 seconds (measured in the e2e journey).
