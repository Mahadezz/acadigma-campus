...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Area             | academics                                                                                                                                                                                           |
| Status           | in progress — demo cut shipped (D-104)                                                                                                                                                              |
...
_Steps:_ the service worker intercepts the action; the payload — including a client-generated **`idempotency_key` (uuid v4, created when the screen opens, not when Save is pressed)** — is written to IndexedDB with a `pending` flag; the UI shows an optimistic "saved" state and a small "1 pending" chip in the top bar. On reconnect, on the next app open, or when the app becomes visible again, the page drains the queue in order, calling the same server action — no Background Sync (D-71: iOS has none, so one code path everywhere). _(2026-09-26, D-71: this queue is the platform-wide outbox of F-ID-11; offline covers the whole app, not only attendance, and F-ID-11 §4–§5 is authoritative for keys, replay, conflicts, late sync and purges.)_ The server dedupes on `idempotency_keys`.
...
**Part 7 — Offline queue and scan hook** · service-worker queue in IndexedDB with client-generated idempotency keys, pending chip and queue sheet, conflict diff sheet, replay on open/online/visible (no Background Sync, D-71), `attendance_scan_events` + `POST /api/attendance/scan` with HMAC verification, "Pre-fill from gate scans" · tests: Playwright offline-mode journey (go offline, mark, save, reconnect, assert exactly one session), replay-attack test on the scan endpoint · **Demo:** turn off the network, take attendance for two sections, turn it back on, and watch both sync with no duplicates. _(2026-09-26, D-71: the offline half of this Part — queue, pending chip, queue sheet, conflict sheet — is built as F-ID-11 Part 2 on the platform-wide outbox; this Part keeps the scan hook.)_
