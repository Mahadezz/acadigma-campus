...
- **Hotfix 2026-09-26 (PR #61):** `/app/classes` returned the generic error for every owner in production. Root cause: `public.workspace_members` has FOUR foreign keys to `public.profiles` (`user_id`, `invited_by`, `removed_by`, `created_by`), so `getClassesOverview`'s `class_teacher:workspace_members!sections_class_teacher_fkey(profiles(full_name))` embed and `listClassTeacherOptions`'s `profiles(full_name)` embed were ambiguous — PostgREST rejected both with `PGRST201`. Fixed by hinting the FK explicitly: `profiles!workspace_members_user_id_fkey(full_name)` (`packages/db/src/repositories/academics.ts:64,129`); confirmed against `pg_constraint` and reproduced/verified via direct PostgREST calls on a local `supabase start` stack. No other `profiles` embed through `workspace_members` exists in the repo. CI could not have caught this: the `unit` job's vitest suite runs with no database at all and `academics.test.ts` mocks the Supabase client in-memory; the `db` job's pgTAP runs against bare `postgres:17` with no PostgREST; and the only e2e journey opening `/app/classes` as an owner (`add-section-and-assign-teacher.spec.ts`) is skip-gated on `E2E_LIVE_SUPABASE` + OQ-27, which CI never sets. Regression test added: `packages/db/src/repositories/academics.integration.test.ts`, an opt-in integration test (`DB_LOCAL_SUPABASE=1 pnpm --filter @acadigma/db test`) that runs the real repository functions against a real local PostgREST — verified to fail with the pre-fix embeds and pass with the fix. Not wired into CI (no CI job runs a full Supabase stack); closing that gap needs either resolving OQ-27 for the live e2e journey or adding a PostgREST-backed job, both out of scope for this hotfix.

### Status / deviations recorded 2026-09-26 (Part 5 demo cut, D-107)

- **Built:** `section_subjects` (migration `20260925300320_section_subjects.sql`, pgTAP `37_section_subjects.sql`, 24 assertions). Each row is section × subject × `teacher_id`, with composite tenant FKs, `unique (section_id, subject_id)`, T2 RLS, audit and the read-only guard. The §5 rule 9 eligibility trigger and the §4.5 release trigger now cover subject teachers too. `public.set_section_subjects` replaces a section's subject list in one transaction. On `/app/classes`, each section row has a **Subjects** sheet (owner/admin): tick a subject, pick its teacher, one Save. Every row shows "n subjects". English and Bangla. `listMySections(supabase, ctx)` (`@acadigma/db`) returns the sections the signed-in member teaches (class teacher or subject teacher) with their subjects, for the basic-mode home (F-ID-10 Part 2, D-403 (c)). `create_exam` now takes each paper's teacher from `section_subjects` (F-AC-06 D-303 item 2 / D-304 item 1).
- **Deviations from §3/§7:** the column is `teacher_id` (DATA-MODEL), not `primary_teacher_id`. There is no `academic_year_id` (it comes from the section), `room_id`, `periods_per_week`, `full_marks`/`pass_marks`, `is_optional_fourth` or `is_active` yet. Taking a subject off a section **deletes** its row, because nothing references one yet; archive arrives with the first referencing table. One action, `setSectionSubjects` (the whole list), stands in for `assignSectionSubjectTeachers`. §4.3 opens a sheet per subject row; the demo cut uses one sheet per section.
- **Not built yet (rest of Part 5):** `section_subject_teachers` (assistants), `bulkAssignSubjectTeacher`, `updateSectionSubjectOwn` and the teacher's own-row update policy, the "n subjects need a teacher" banner, `academics.assigned_subject`/`academics.subject_unassigned` notifications, and copying subjects from `grade_level_subjects` when a section is created (Part 4).
- **Tests:** `37_` (set/replace/remove, eligibility, composite FKs, isolation, escalation, parent and anon, create_exam default, release on removal, audit, read-only). Unit tests cover the contract, repository, action and sheet. `academics.integration.test.ts` runs `setSectionSubjects` and `listMySections` against a real local PostgREST. The journey `add-section-and-assign-teacher.spec.ts` gains a subjects test (skip-gated on OQ-27).
