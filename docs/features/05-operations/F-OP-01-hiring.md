...
| Browse opted-in teacher profiles              | `hiring.candidates.browse`     |  ✅   |  ✅   |    —    |   —   |   —    |    —     |                         —                          |
...
| `source`                                       | enum `application_source`  | `public_page \| personal_area \| admin_entry \| invited_from_browse`           |
...
**`open_to_work = true AND visibility = 'active'` is the only combination a school can see.** Files listed here are `files.visibility='private'`; nothing but an approved `document_requests` row unlocks them for a school (§5.6).
RLS: owner of the row has full access; **schools have no direct select** — browse goes through a `security definer` function `app.browse_teacher_profiles(filters)` that returns only the safe projection (no file ids, no phone, no email, no exact salary unless `salary_visible`).
...
Trigger: admin taps **Request documents** on a dossier or a browsed profile.
...
### W8 — Browse candidates

Trigger: admin opens tab _Candidates_ (Pro entitlement).

1. Filters: subject, grade level, years of experience, availability, preferred work type, location, verified-only, minimum profile score. Server-side filtering, cursor pagination (`DataList`).
2. Results are the safe projection only. **Contact details and files are never in the payload.**
3. Actions per profile: **Invite to apply** (picks an open posting → creates `applications{source:'invited_from_browse', stage:'applied'}` only _after_ the candidate accepts the invitation notification — a school cannot create an application on someone's behalf) and **Request documents** (W7).
   Failures: zero results shows the honest empty state ("No teachers match — 412 profiles are open to work this month; try widening subjects"), not Base44's permanently-empty list.
...
| Candidate browse  | `/app/hiring/candidates`          | Filter sheet (bottom) + result cards                                                                           | Filter rail + card grid                                          | Invite to apply                             | Upgrade card when the plan lacks `hiring`                                                              |
...
| `browseCandidates` (route handler, GET)   | `BrowseCandidatesQuery`                                                                                              | `Page<CandidateCard>`                 | `plan_required`                                                             | —                                       | 120/h                   |
...
**Part 8 — Teacher profile, `open_to_work`, profile score, candidate browse** · `/personal/cv` editor with live score and "what's missing", visibility/availability switches, `app.browse_teacher_profiles` projection, filters + pagination, invite-to-apply.
_Demo:_ a teacher fills their profile to 85, flips `open_to_work`, and appears in another school's browse within one refresh — with no email, phone or file id anywhere in the network payload.
...
**Teacher profile and browse** 24. _Given_ a profile with photo, 150-char bio, 2 subjects, 1 education entry, CV, experience, 1 grade level, 1 work type, 1 location, 1 language and an approved identity verification, _then_ `profile_score = 70 + 10 = 80`. 25. _Given_ `open_to_work = false`, _when_ a school browses candidates, _then_ that profile is absent from the results and from the API payload. 26. _Given_ any browse response, _when_ the network payload is inspected, _then_ it contains no email, phone or `file_id`.
...
- **Performance budgets**: `/jobs/[slug]` LCP ≤ 1.8 s on Slow 4G (it is a public, SEO-relevant page); pipeline first render ≤ 1.2 s p95 for 200 applications; `browseCandidates` p95 ≤ 400 ms server time at 5,000 profiles.
