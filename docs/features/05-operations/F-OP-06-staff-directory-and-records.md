...
Every other operations feature assumes there is a row that says _this person works here, in this role, from this date, at this rate_. **Staff Directory and Records** is that row, plus the screens around it: a searchable list of everyone in the school with their designation and subjects; a record per person holding employment details, documents and pay (admin-write, admin-and-self-read); a teacher-facing **"was I paid this month"** payslip view built entirely from data this feature already holds (§4 W4, R1); and an offboarding checklist that, when completed, actually removes access instead of leaving a former employee with a live login.
...
- One row per change; `effective_to` is closed by a trigger when a newer row is inserted; no overlaps (exclusion constraint on `(staff_record_id, daterange(effective_from, effective_to))`) — the constraint does not prevent a GAP (a period with no rate at all).
...
- The engine reads through `app.staff_hourly_rate(workspace_id uuid, user_id uuid, on_date date) returns bigint` — `SECURITY DEFINER`, `STABLE`, search_path pinned, returning only the single number for the given date within the named workspace, and **only** when the caller is that same person or `{owner,admin}` of that workspace — **never** a teacher or staff colleague, and never mixed across a person's other workspace if they hold more than one staff record (D-63 items 7 and 9). Nothing else may read the table on a teacher's behalf.
...
- **From an invitation**: inviting a member offers "also create a staff record" (default on for `teacher`/`staff`/`admin` roles). The invitation itself carries the **staff privacy notice** (F-ID-04 §4.1, `legal_acceptances{document:'staff_privacy_notice'}`) — this feature does not re-show it, but a record cannot leave `pending_join` without that acceptance existing.
...
### W4a — "Was I paid this month?" (teacher payslip view, R1)

Trigger: a teacher/staff member opens their own **Pay** tab (part of their own profile at `/app/staff/me` or `/personal/pay` — same route family as F-TE-06's own-workload cockpit, so "my numbers about me" live in one predictable place).

This is **not a new payroll system** — it is a read view assembled entirely from rows this feature and F-OP-02 already hold: `staff_compensation` (this member's own rows, already `own`-readable per §2) and `cover_assignments` (already the source of `extra_hours` in F-OP-02 §5.5 and F-TE-06 §5.8).

1. **This month, at a glance:** for a **salaried** member (`monthly_salary_paisa` set), the figure shown is that month's `monthly_salary_paisa` at whatever rate was `effective` on the 1st of the month. For an **hourly** member (`hourly_rate_paisa` set), the figure is `Σ cover_assignments.minutes where cover_teacher_id = self and date in [month_start, month_end] and status='completed'` converted to hours, multiplied by the `hourly_rate_paisa` **in force on each cover date** (§5.8 — the same historically-correct rate F-OP-02 already uses, read through `app.staff_hourly_rate(user_id, on_date)`, never today's rate applied retroactively).
2. **History:** the same computation for prior months, oldest-first-collapsed, so a teacher can answer "was I paid for October's cover" without asking the office.
3. **Explicit disclaimer, always visible:** _"This is computed from your rate and recorded hours — it is not a payslip issued by your school's accounts department and does not reflect tax, advances or manual adjustments the office may apply."_ The view is informational, never a legal payslip, and never claims to be one.
4. **Nothing here is writable.** No action on this screen changes `staff_compensation` or `cover_assignments` — a discrepancy is something the teacher raises with the office, not something this view lets them self-correct.
   Failures: no compensation row at all → "Your rate hasn't been set yet — ask your school office"; a month with zero completed cover for an hourly member → "৳0 this month" shown plainly, not hidden, so an unpaid month is never silently blank.

The computation is exact in §5.8.
...
- **Derived metrics are staff-visible and advisory-only [2027-05].** Any workload, punctuality or eligibility-adjacent metric computed about a staff member (e.g. F-TE-06 §5.3 workload) is visible to that staff member wherever it is computed — this feature does not hide a colleague's own numbers from them. The written statement staff see at invitation and in Settings (F-ID-04 §4.1) applies here too: these metrics are advisory only and are never used to make an automated employment decision.

### 5.8 Payslip computation (W4a, R1)

```
salaried (monthly_salary_paisa is not null):
  this_month_paisa = monthly_salary_paisa of the staff_compensation row effective on month_start

hourly (hourly_rate_paisa is not null):
  this_month_paisa = Σ over completed cover_assignments c where c.cover_teacher_id = self
                        and c.date between month_start and month_end
                      of (c.minutes / 60) × app.staff_hourly_rate(self, c.date)
```

`app.staff_hourly_rate(user_id, on_date)` is the same `SECURITY DEFINER` function §3.2 already defines for F-OP-02's cover payroll — the payslip view is a second reader of an existing function, not a new rate-lookup path. Both branches only ever read `staff_compensation` and `cover_assignments` rows already scoped to `self` by RLS (§2, §3.2); the payslip server action performs no query a teacher could not already run against their own rows individually — it is a convenience aggregation, not a new grant of visibility.
...
| My pay (payslip view, R1) | `/app/staff/me` (or `/personal/pay`), self only | This-month figure card + disclaimer + collapsed monthly history                  | Same                               | none — read-only     | "Your rate hasn't been set yet — ask your school office" · "৳0 this month" shown, never hidden |
...
| `getMyPayslip` (route, GET, self only, R1)      | `{ month?: 'YYYY-MM' }`                                                          | `{ month, kind:'salaried'\|'hourly', totalPaisa, breakdown?: CoverLine[] }` | `no_rate_set`                                       | —                          | 300/h      |
...
**Part 4 — Compensation + custom labels** · the compensation section with history and the new-rate flow, the self-view, **the teacher-facing "was I paid this month" payslip view (W4a, §5.8) built from `staff_compensation` and `cover_assignments` alone**, `custom_labels` CRUD with drag ordering and the Pro gate, the "permissions never read labels" test.
...
**Payslip view (R1)** 12a. _Given_ an hourly teacher with ৳350/h effective all of September and 6 completed cover assignments in September totalling 9 hours, _when_ they open their payslip view for September, _then_ it shows ৳3,150 and the disclaimer text. 12b. _Given_ a rate change mid-month (৳350/h through the 15th, ৳400/h from the 16th) and cover on both sides of the change, _when_ the payslip is computed, _then_ each cover assignment is priced at the rate in force **on its own date**, not a single month-end rate applied to all of them. 12c. _Given_ a salaried member, _when_ they open their payslip view, _then_ it shows that month's `monthly_salary_paisa` from the row effective on the 1st, with no cover-hours computation performed. 12d. _Given_ any teacher, _when_ they open another member's payslip route by guessing or editing a URL, _then_ they get `forbidden` — the view is self-only, enforced server-side, not just hidden in the UI. 12e. _Given_ the payslip view, _when_ inspected for any write affordance, _then_ none exists — every action on the page is read-only.
...
- **§3.1's compensation-table-vs-column question is answered:** `staff_compensation` is a separate table, keyed by `staff_record_id` (not `(workspace_id, user_id)`), `effective_to` inclusive. `app.staff_hourly_rate(workspace_id, user_id, on_date)` checks the caller's own standing against that named workspace: owner/admin of it, or the row's own person — **never** a teacher or staff colleague (D-63 item 7 — a first draft allowed any active member and was a confirmed leak, caught by CI's live pgTAP run, not by the design read).
...
- **All three new tables were missing their explicit `revoke all ... from anon, authenticated`** (D-63 item 8, the table-grant equivalent of D-54) — not a data leak (RLS already blocked `anon`), but a real grant-hygiene gap, caught by CI and fixed before merge.
- **`app.staff_hourly_rate` gained a third argument, `p_workspace_id`, restored after a lead review** (D-63 item 9): a 2-argument version resolving "the" workspace implicitly from the staff record risked mixing schools' rates for a person staffed at more than one (AC-30 pgTAP now proves this directly, not just "implied" by tenant isolation).
