...
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Area             | ops                                                                                                                                                                                                                                       |
| Status           | in progress — Parts 1-2 shipped (D-204, D-205); Part 3 shipped, real seam since F-AC-06 Part 5/#68 (D-206, D-305); Part 5 shipped as a demo cut, merge/duplex only, no split files or publish, real seam (D-207); Part 6 shipped as a demo cut, the two templates only, `attendance_register_signoffs` deferred (D-208); Part 4, 7-8 not started |
...
14. **PDF byte integrity (2026-09-26).** A suspected stream corruption after #62 was traced to how sample files were saved (bytes decoded as UTF-8 text and written back one byte per character, turning every non-ASCII byte into `0xFD`), not to the renderer or the `serverExternalPackages` change: `next build` + `next start` output and a direct Node render both inflate cleanly. `packages/pdf/src/pdf-streams.test.ts` now inflates every `FlateDecode` stream of a real render with `node:zlib` and checks the Bengali text layer. The same investigation found the danda (`।`, U+0964/U+0965, Devanagari block) was drawn in Inter as a tofu box; `ScriptText` now routes it to Hind Siliguri.

### Addendum — Part 3 as shipped (D-206)

15. **The report card renders from a fixture, not F-AC-06's `results` (written by `app.compute_results`), until F-AC-06 Part 5 lands.** §8 Part 3 names the real grading function; it depends on `marks`/`results` tables that are not on `main` yet (built in parallel in another lane). `ReportCardParams` already carries real `studentId`/`examId` uuids, and one function (`getReportCardData`, `apps/web/app/(school)/app/reports/report-card-data.ts`) is the only place any caller resolves a `ReportCardDto` — swapping the fixture for the real query touches that one function's body only. See D-206.
16. **Grading is #46/D-302's shipped `grade_scales`/`grade_bands` domain (`bandFor`, `BD_GRADE_BANDS`), not the spec §3's originally proposed `grade_scale_bands` shape.** D-302 already resolved that naming disagreement for F-AC-06 Part 1; this Part reuses the same TypeScript functions rather than inventing a second grading implementation. `packages/pdf` still contains no grade-band logic of its own (§5.1) — the fixture computes every letter/GPA before the DTO reaches the template.
17. **One A4 portrait layout only, no attendance register or mark sheet yet, no comments, no signature-label customisation from `school_profiles.signature_labels`** (owner instruction: keep it lazy). Signature labels are locale-default text ("Class Teacher"/"Guardian", "শ্রেণি শিক্ষক"/"অভিভাবক") baked into the template, not read from branding. Rank is computed within the fixture's own 40-student class, not from a real `results` table's section ranking.
18. **No bulk report cards, no publish-to-parent flow, no `report_run_items` rows.** This Part renders exactly one student's card per run, synchronously, the same "immediate in-request kick" pattern items 9-10 above already established for `'sample'`.
19. **Review fixes (2026-09-26, D-206 items 5 and 7).** Title "প্রগতিপত্র" / "Progress Report". The DTO follows F-AC-06 `results`: `incomplete`/`withheld` results print "অসম্পূর্ণ — ফলাফল স্থগিত" and hide totals, percentage, GPA, grade, pass/fail and rank; subject lines carry `status` and `subjectKind`; the card carries `gpaWithoutOptional` and `rankTied`. Staff may open reports and render a report card (a lead decision that differs from §2's "—"); a teacher's class-teacher limit is enforced once real data lands. The seam is `getReportCardData(supabase, ctx, studentId, examId)` and never serves the fixture in production.

### Addendum — Part 5 as shipped, demo cut (D-207)

20. **Split files and publish-to-parent are explicitly not built.** §8 Part 5 names "merged + split output... the publish/unpublish flow and the parent-side list" — this cut ships only the merged PDF. Publishing to parents belongs to F-AC-06 Part 7 (task instruction). `report_publications` is not touched.
21. **`report_card_bulk` is a new `report_kind` value, not a widened `report_card` params object** — the params genuinely differ (`sectionId`/`order`/`duplex` vs. one `studentId`) and the spec already names a distinct permission key (§2). See D-207 item 1.
22. **`report_run_items` rows are written once, after rendering, not pre-created before rendering starts.** §8 Part 5's own AC10 ("34 `report_run_items` exist before rendering starts and progress reports 'n of 34'") describes a progress UI this synchronous demo cut does not build (no drainer, no Realtime, same D-205 precedent every kind so far has used). Every roster id is still attempted through the seam and every outcome (success or failure) is recorded — nothing is silently dropped — but "before rendering starts" is not literally true here.
23. **Duplex padding pads after every card whose running page count is odd, unconditionally including the last one.** For this Part's one-page-per-card template, that means every card gets a fixed 2-page block (content + blank) — 40 cards duplex-pad to exactly 80 pages, not 79. See D-207 item 2 and `packages/pdf/src/merge.test.ts`.
24. **The bulk button lives on the results preview, next to each row's single-card button.** `/app/exams/[id]/results`, "Print all report cards", same `report.render.report_card_bulk` gate, real `sectionId`/`examId` — moved there after F-AC-06 Part 5 (PR #68) merged and this branch merged `origin/main` (D-207 item 8, review update). `/app/reports` carries no report-card button, single or bulk.
25. **Roster resolution is a second, narrow seam (`getReportCardBulkStudentIds`), not the DTO seam.** Real since #68: it calls `getSectionResults(ctx, client, examId, sectionId)` and returns every `studentId` in its rows, unfiltered. The bulk render step still calls `getReportCardData` once per id — a student the seam refuses (no paper counted at all, D-305 item 9; a missing roll number alone still gets a card printed with "—") is a per-student `failed` `report_run_items` row, never a silent drop. See D-207 item 4.
26. **New dependency: `pdf-lib` 1.17.1** (`packages/pdf`), for merging already-rendered single-student PDFs and inserting duplex pad pages — no already-installed package does PDF byte-stream merging. See D-207 item 2.
27. **Security hardening bundled into this Part's migration** (task's own instruction, security LOW from the #63 review): `report_runs_select`/`report_run_items_select` now also require `kind = 'report_card' OR has_role(...,{owner,admin,teacher})`, so staff can never see a non-`report_card` run even in a state the insert policy alone would not create. See D-207 item 7.

### Addendum — Part 6 as shipped, demo cut (D-208)

28. **`attendance_register_signoffs` (§3.1a), the sign/countersign/reopen actions (§4 W7), the `PERIOD_LOCKED` guard (§5.2a) and AC26a-26e are NOT built.** The task's own instruction named this deferral explicitly ("unless it's tiny") and D-208 explains why it is not tiny: a three-state machine, three server actions, a cross-feature write guard on `attendance_records`, and five of its own acceptance criteria. Tracked for a future Part.
29. **Two new `report_kind` values, migration is `alter type ... add value` only** — `attendance_register`, `mark_sheet`. No new table, no RLS change; both kinds are single-item, synchronous renders exactly like `report_card` (D-205's precedent). See D-208 item 1.
30. **The register's columns are every calendar day of the month, not only school days** — non-school days are greyed in place rather than omitted, so the grid's day-of-month numbering matches a real paper register. Status codes on the grid are the plain Latin letters P/A/L/E/H in both locales (a legend below the table carries the Bengali meaning); a deliberate simplification because several Bengali status words collide on their first letter. See D-208 item 4.
31. **No new `app.*` SQL function for calendar data.** `app.is_school_day`/`app.school_days` are not exposed via PostgREST (`app` schema, not in `supabase/config.toml`'s exposed schemas) — `getAttendanceRegister` instead reads `school_profiles.working_days`, `holidays` and `working_day_overrides` directly (already SELECT-able by an active member) and recomputes the same override > weekly-pattern > holiday > true precedence in TypeScript (`@acadigma/domain/calendar`). See D-208 item 3.
32. **The mark sheet is built entirely from F-AC-06's already-computed `getSectionResults`** (D-305) — no second grading/averaging implementation anywhere in `packages/pdf` or its data path, matching §5.1's "one grading truth" rule. Column order is alphabetical (inherited from `getSectionResults`'s own sort), not the exam's configured paper order §5.3 names. See D-208 item 5.
33. **"Results not computed" is a run failure (`no_data`), not an inline per-sheet banner.** An exam whose results were never computed at all refuses the whole render with a clear message; a student whose own result is `incomplete`/`withheld` still renders a row, printing "অসম্পূর্ণ"/"Incomplete" in place of GPA/grade/rank, exactly like the report card already does. See D-208 item 6.
34. **Permissions asserted for real, not an interim role grant.** `report.render.attendance_register` and `report.render.mark_sheet` (owner/admin/teacher, not staff) rely on each underlying table's actual current RLS (attendance: any teacher, D-105's precedent; results: the class-teacher scoping F-AC-06 already enforces) rather than a fixture-era placeholder. See D-208 item 7.
35. **Buttons live where the data already is:** the register's button is on the roll-call page (`/app/attendance/[sectionId]`), defaulting to the viewed date's month; the mark sheet's button is on the results preview (`/app/exams/[id]/results`), next to the report-card buttons. Neither is on `/app/reports`. See D-208 item 8.
