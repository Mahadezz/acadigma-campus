...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Area             | ops                                                                                                                                                                                                                         |
| Status           | in progress — Parts 1-2 shipped (D-204, D-205); Part 3 shipped fixture-backed (D-206); Part 5 shipped as a demo cut, merge/duplex only, no split files or publish (D-207); Part 4, 6-8 not started                          |
...
24. **The bulk button lives on `/app/reports`, not the results preview.** The lead's review named the spec-correct home as the exam results preview's "Print all report cards" action (`/app/exams/[id]/results`, F-AC-06 Part 5, PR #68) — that page is not on `main` yet. `GenerateReportCardBulkButton` on `/app/reports` is a temporary, fixture-guarded demo entry point, same pattern as D-206's single-card button, expected to move once PR #68 merges. See D-207 item 8.
25. **Roster resolution is a second, narrow fixture seam (`getReportCardBulkStudentIds`), not the DTO seam.** The bulk render step still calls `getReportCardData` once per student id — a student the seam refuses (missing data today; a real result with no roll number once F-AC-06 Part 5 lands, D-305) is a per-student `failed` `report_run_items` row, never a silent drop. See D-207 item 4.
