...
9. **EIIN is read-only in Settings (D-100, PR #37).** The profile form shows it with a "contact support to change" note and `schoolProfilePatchSchema` omits it; a client UPDATE of `school_profiles.eiin` is refused by `app.tg_school_profiles_eiin_guard`, because it was an unthrottled EIIN oracle.
