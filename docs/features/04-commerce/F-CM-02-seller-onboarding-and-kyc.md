...
| `decision_reason_code`               | `kyc_reason` null                         | `illegible \| name_mismatch \| expired_document \| selfie_mismatch \| suspected_fraud \| incomplete \| other` |
...
- _select_: the owning `user_id` sees their own rows **excluding** `internal_note` (enforced by a column-level grant revoke plus the repository's explicit column list); platform staff see everything. **No other role, in any workspace, at any time, can select a single row** — there is no school-admin policy, no membership-based policy, nothing.
...
3. Three decisions, each in a confirm sheet:
   ...
   - **Reject** → submission `rejected`, profile `kyc_status='rejected'`, `status='rejected'`. Seller may appeal by contacting support; a new attempt is allowed after 7 days.
