...
| Plan             | `docs/plan/ROADMAP.md` — **P1–6 moved inside the R1 launch gate (M3)**, ahead of the marketplace chunks; **P7–14 stay R1.5** (later M4)                                                                                                                                            |
...
>
> **Scope note (research-debate synthesis, `docs/product/research/DECISION-CHANGES.md` §2).** Parts 1–6 (fee heads/structures, discounts, invoicing, offline collection, late fees — see §8) ship **inside the R1 launch gate (M3)**, not M4/R1.5 as originally scoped: a school will not go live on Acadigma without fee collection. Scope added for the R1 gate: a **defaulter call sheet** for print/desk use, guardian name and phone, **sorted by amount** (§4.8); a waiver/discount workflow that **requires an approver** above threshold (§4.2, §5.6 — already modelled); **sibling discounts** as a first-class `discount_kind` (§3.4 — already modelled); and a new **admit-card / result-withholding dues gate** — "block admit card / withhold result if dues > X" (§4.12). Parts 7–14 (online payment, IPN, bKash adapter, reminders) stay **R1.5**, later M4.
...
`workspace_id pk`, `default_due_day int not null default 10`, `grace_days int not null default 0`, `late_fee_enabled bool not null default false`, `late_fee_method` (`flat | percent_of_outstanding | per_day`), `late_fee_flat_paisa`, `late_fee_percent_bps`, `late_fee_per_day_paisa`, `late_fee_cap_paisa`, `late_fee_max_days int`, `late_fee_head_id`, `allow_partial_payment bool not null default true`, `min_online_payment_paisa bigint not null default 1000`, `allocation_strategy` (`oldest_first | head_priority`), `waiver_approval_threshold_paisa bigint not null default 500000`, `public_pay_enabled bool not null default false`, `receipt_language` (`bn | en | both`) default `both`, `receipt_prefix`, `receipt_footer_bn`, `receipt_footer_en`, `institution_seal_file_id`, `signatory_name`, `signatory_designation`, `reminder_days_before int[] default '{3}'`, `reminder_days_after int[] default '{1,7,15}'`, `block_admit_card_dues_threshold_paisa bigint null` (R1 gate; `null` = feature off), `withhold_result_dues_threshold_paisa bigint null` (R1 gate; `null` = feature off), `updated_by`, `updated_at`.

**§5.12 note:** `public_pay_enabled` now defaults **off** — a school must consciously turn on the unauthenticated public payment page (§5.12).
...
1. A plain-language explainer first: _"Fees are paid into **your** school's account, not Acadigma's. Acadigma never holds your money and takes no commission on fees."_ **Default rail: bKash Merchant Account (~1.85% MFS) — pending owner (OQ-21)** on the final gateway default. SSLCommerz (৳25,500 one-time plus ~2.5% per transaction, COMPETITORS.md §6.4) is offered as the card-accepting alternative for schools that need it. For a school that cannot hold a merchant account at all, two documented fallbacks exist and do not require §4.5–§4.6's online-checkout path: (a) a **bKash Personal/Retail Account** used as a manual collection point, reconciled by the office like any other bank/cash channel in `fee_transactions`; (b) a **bKash-statement CSV import** (§4.11) that lets the office upload their bKash statement and have candidate rows matched to students by phone/amount/reference for review before posting as offline payments. A school deserves to know these trade-offs before it starts.
...
4. **IPN** hits `/api/fees/ipn/<webhook_path_token>` → resolve the merchant account from the token → write `inbound_events` (idempotent on the **`(store_id, tran_id, val_id)` triple**, not `val_id` alone — a single provider event id is not enough replay protection once multiple schools' events can collide) → recompute `verify_sign` with **that school's** store password → **call the validation API with that school's credentials** → check `tran_id`, exact paisa amount and `BDT` currency against our `fee_payments` row, **and that the resolved `store_id` matches the merchant account the webhook token resolved to** (a callback landing on school A's token must never validate against school B's `store_id`) → only then capture.
...
`/app/fees/defaulters`: students with `balance_paisa > 0` on an invoice past `due_on + grace_days`, **sorted by overdue amount descending by default** — student, class, guardian phone (a `tel:` link; the office's next action is almost always a call), overdue amount, days overdue, last payment, last reminder. Filters by class/section, days-overdue band, amount band. Actions: **Send reminders**, **Export**, **Print call sheet**, **Open ledger**.

**Call sheet (R1).** _Print call sheet_ renders a print-optimised PDF/HTML sheet for the front desk to work down while phoning defaulters: student name, class, guardian name, guardian phone, overdue amount, days overdue — one row per student, **sorted by amount descending**, no ledger detail, no discount/waiver history. It is a working document for a desk, not a parent-facing artefact, and is not watermarked or receipted.
...
1. Select recipients (from the defaulter list or a filter) and channels. In-app and email are free; **SMS is metered and priced at ৳0.75 per UCS-2 segment** — never bundled as "unlimited," and the SMS cost is **always quoted separately from the subscription price**, never folded into it.
2. Plan allowances: **300 segments/month included on Pro; none included on Starter** (every Starter SMS is billed from the first segment); Enterprise is contractual. Included segments and top-ups are drawn from `sms_credits_ledger`, the **SMS** credit pool — a separate pool from F-TE-03's AI-action credits, never conflated.
3. An **estimate step is mandatory before every send and cannot be skipped**: _"312 recipients · 2 segments each (Bengali) · 624 segments · about ৳468 · your balance is 1,450 segments."_ If the balance is short the send is **blocked entirely**, never partially delivered.
4. Templates are per school, bilingual, with merge fields (`{{student_name}}`, `{{class}}`, `{{amount}}`, `{{due_date}}`, `{{school_name}}`) and a **live segment counter**, wired to the same price list the estimate step reads (§5.2-shaped: one source of truth, not a hardcoded constant in the composer) — Bengali is UCS-2, so a Bengali SMS is **70 characters per segment, not 160**, and a 200-character Bengali message costs three segments. The counter shows segments and taka, not characters, because that is the number that matters.
5. Sending debits `sms_credits_ledger` per segment the provider accepts; failures refund the ledger.
6. Scheduled reminders run from `reminder_days_before` / `reminder_days_after`, with at most **one SMS per guardian per day** across all reminder types, enforced against `fee_reminders`.
7. A guardian who has opted out is `suppressed` with a reason and costs nothing.
...
- **bKash-statement CSV import (later M4).** For a school on the Personal/Retail Account fallback (§4.5): the office uploads a bKash statement export; the importer matches rows to students by phone number, amount and reference text, and stages each as a **proposed** `recordOfflinePayment` call the office reviews and confirms row-by-row — nothing posts automatically. Unmatched rows are listed for manual assignment. This is an import aid, not a new payment channel: every posted row still goes through the one write path (§3.14).

### 4.12 Admit card / result hold (dues gate, R1)

A school may set `fee_settings.block_admit_card_dues_threshold_paisa` and/or `withhold_result_dues_threshold_paisa`. When a threshold is set (non-null) and a student's current `balance_paisa` across active invoices exceeds it, the student is flagged **dues-blocked** to the consuming feature (exam admit-card generation, result/report-card release — owned by those features, which read this flag rather than duplicating fee logic). Rules:

- **Off by default** (`null` threshold) — a school must consciously enable a hold that could keep a child out of an exam.
- An **admin/owner override** is available per student with a required reason, audited as `fee_hold.overridden`, so a genuine hardship case is never mechanically blocked.
- The block is evaluated **live** at the moment admit cards or results are generated, never cached, so a payment made minutes earlier clears it.
- The parent portal and the office both surface **why** a student is blocked (_"Admission Test admit card withheld — ৳4,500 overdue"_), never a silent failure.
...
### 5.12 Public student-ID lookup: what it may reveal [MUST — before M4 4.3]
...
- **`fee_settings.public_pay_enabled` defaults OFF.** A school must consciously turn the public page on; until then, online payment is logged-in-only (§5.10's "no dead button" rule still applies — the parent portal explains where to pay instead).
...
- Rate limited to **10 lookups per IP per hour** and 3 per student ID per hour **per IP**, with a CAPTCHA after 3 failures. **In addition, a GLOBAL lockout applies per student code across all IPs combined** — once a student code accumulates a configured number of failed attempts from any mix of IPs in a rolling window, that student code is locked out platform-wide for a cooldown period, closing the obvious bypass of rotating IPs to keep guessing one child's guardian-phone digits.
- **Wrong code and unknown student are indistinguishable.** Both failure modes — a student ID that does not exist, and a student ID that exists but the guardian-phone last-4 is wrong — return the **identical response body and shape**, and the handler is written to take the **same wall-clock time** for both paths (e.g. a constant-time comparison plus a fixed minimum response latency), so neither the JSON shape nor the response timing can be used as an oracle to distinguish "no such student" from "wrong code."
- **Every attempted lookup is logged with the code that was attempted** (workspace, student ID attempted, guardian-phone-last-4 attempted, IP, result, timestamp) — not just successful or rate-limited ones — so abuse review can reconstruct a guessing pattern after the fact.
- **A spike in failed attempts triggers an alert to the school** (in-app + email to owner/admin), separate from the generic rate-limit response, so a school under active enumeration finds out even if no single IP crosses the per-IP threshold.
...
| `lookupStudentForPublicPay`                 | route `POST /api/fees/public/lookup`          | `{schoolSlug, studentCode, guardianPhoneLast4}`                                                                                      | `{lookupToken, maskedName, className, outstandingPaisa, splitRequired}` — identical shape/timing on `NOT_FOUND` vs wrong code | `NOT_FOUND`, `RATE_LIMITED`, `PUBLIC_PAY_DISABLED`, `STUDENT_LOCKED`                                                 | n/a                                  | 10/hour/ip, 3/hour/student/ip, + global per-student-code lockout across all IPs |
...
| `feeIpn`                                    | route `POST /api/fees/ipn/[webhookPathToken]` | form body                                                                                                                            | `200` always                                                                                                                  | never 4xx on a parseable body                                                                                        | `(store_id, tran_id, val_id)` triple | 300/min/ip                                                                      |
...
Tests: replayed IPN captures once, keyed on the `(store_id, tran_id, val_id)` triple rather than a single provider event id; wrong amount → `disputed`, nothing recorded; an IPN on the wrong workspace's token rejected; **a callback delivered on school A's webhook token that validates successfully against school B's `store_id` is rejected and recorded as a cross-tenant mismatch, never captured** (the specific confusion attack, not just a generic wrong-token test); the office-took-cash-meanwhile race leaves a credit rather than an over-allocation.
...
Scope: `/family/fees` with child switcher, Bengali-default rendering, invoice/receipt downloads and discount display; `/pay/[schoolSlug]` with the guardian-phone second factor, starved response, per-IP **and global per-student-code** rate limits/lockout, CAPTCHA, per-attempt code logging, the school spike alert, identical failure-shape/timing for wrong-code vs unknown-student, and `public_pay_enabled` defaulting **off**.
Tests: a guardian sees only their own children (pgTAP via `app.is_guardian_of`); a draft invoice is invisible to a guardian; enumeration is rate-limited and logged; the public response carries no field beyond the four allowed (response-shape freeze test); a wrong-code and an unknown-student attempt produce byte-identical response bodies and statistically indistinguishable timing (no oracle test); a student code attacked from many rotating IPs is still globally locked out; a spike in failures fires the school alert exactly once per spike window.
...
Scope: `fee_reminder_batches`, `fee_reminders`, `sms_credits_ledger`; bilingual templates with the merge-field allowlist, wired to the single price list (**৳0.75/UCS-2 segment**, 300 segments/month included on Pro, none on Starter) rather than a hardcoded constant; the **UCS-2 70-character** Bengali segment counter and taka estimate; the mandatory, unskippable estimate step and insufficient-balance block; scheduled before/after-due reminders; the one-SMS-per-guardian-per-day cap; opt-out suppression; SMS credit pack purchase on Acadigma's merchant account (pass-through).
Tests: Bengali segment counting (70, and 67 with UDH) against known strings; the estimate (segments × ৳0.75, net of any included Pro allowance) matches the debit; failures refund the ledger; the daily cap holds across reminder types; a template with a disallowed merge field is rejected; a Starter-plan estimate shows zero included segments.
**Demo:** estimate 312 Bengali reminders (624 segments) at ~৳468, send, and see the ledger debit match exactly.
...
18. **Given** an IPN posted to school A's webhook token carrying school B's `tran_id` **then** nothing is captured and the event is recorded as a mismatch. **Given** a callback for school A that validates successfully against school B's `store_id` (the cross-tenant confusion case, not just a mismatched `tran_id`) **then** capture is refused and the mismatch is recorded — this is a required, explicitly-named test, not just implied by the wrong-token case.
...
23. **Given** the public pay page **when** a correct student ID is entered with a wrong guardian phone last-4, and separately when a student ID that does not exist is entered, **then** both produce the **identical response body and shape and statistically indistinguishable timing** — no oracle distinguishes "wrong code" from "no such student"; **and when** correct **then** the response contains only masked name, class and outstanding amount — verified by a response-shape test. Every one of these attempts is logged with the code attempted.
24. **Given** 11 public lookups from one IP in an hour **then** the 11th is rate-limited and logged; **given** the same student code is probed from many different IPs such that the global per-student-code threshold is crossed **then** further lookups for that code are locked out platform-wide regardless of IP, and the school receives a spike alert.
25. **Given** `fee_settings.public_pay_enabled` is unset on a newly onboarded school **then** the public pay page is off by default and must be explicitly turned on before it is reachable.
26. **Given** 312 Bengali reminder recipients **when** the batch is estimated **then** segments are counted at **70 characters per segment** (UCS-2), the cost is shown at **৳0.75 per segment** before sending, and with insufficient credits/allowance the send is blocked entirely rather than partially delivered.
27. **Given** a guardian who already received a fee reminder today **when** a second batch targets them **then** they are suppressed and no credit is spent.
28. **Given** a gateway refund **when** it settles **then** allocations reverse, invoice balances recompute, a Bengali refund receipt is issued, and the money returns from the **school's** merchant account — with no Acadigma refund row created.
29. **Given** the nightly invariant check **when** allocations and invoice `paid_paisa` disagree **then** the school is shown the discrepancy with the offending ids and the job does **not** self-heal.
30. **Given** a school on the **Free** plan **when** a member opens `/app/fees` **then** it returns 404 (the `fees` module is Starter and above); **and given** they downgrade from Starter to Free **then** every fee row is preserved and exportable while new invoice runs are blocked.
31. **Given** any parent-facing fee screen or PDF at 360×800 **then** Bengali renders correctly at ≥ 16 px, the outstanding figure and Pay action are thumb-reachable, and axe reports zero serious violations.
...
9. **bKash merchant API access, and the default gateway (pending owner — OQ-21).** bKash Checkout needs a merchant agreement and API credentials per school. _Default assumed:_ bKash Merchant (~1.85%) is the default rail; SSLCommerz (~2.5%) is the card-accepting alternative, deferred to M4 for schools that need it. A school without any merchant account uses the Personal/Retail Account path or the bKash-statement CSV import (§4.5, §4.11) instead. **Final gateway choice is an owner decision — OQ-21 — not yet made.**
