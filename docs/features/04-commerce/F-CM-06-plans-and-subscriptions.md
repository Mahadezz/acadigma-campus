...
- The route is `GET`+`POST`, not `POST` alone as §7 names it: Vercel Cron Jobs only ever call a route with `GET` (no method is configurable), adding `Authorization: Bearer $CRON_SECRET` itself. `POST` is kept for a manual/scripted trigger with the same header.
