...
- Shipped: `public.expire_pro_trials()` (`20260925000500_trial_expiry_billing_tick.sql`) and `GET /api/cron/billing/tick`, run daily at 09:00 Asia/Dhaka (`03:00` UTC, `apps/web/vercel.json`) — a Vercel Hobby-plan cron only runs once a day, so the T-3/T-7 multi-point schedule below is not attempted from this cron config. A school-shell banner (`apps/web/app/(school)/app/layout.tsx`) reads `access_mode` on every request and explains the read-only state.
- Not shipped in Part 4 (deferred to whichever Part adds the pieces they depend on): the T-3-days trial-ending-soon email + banner (needs an email adapter — none exists yet), scheduled-downgrade application (needs `changePlan`/Part 5's downgrade-scheduling, unbuilt), and the one-email-per-day dunning guard (needs the renewal/dunning ladder, Part 7, unbuilt). `runSubscriptionJobs`'s response is `{trialsExpired}` only, not §7's fuller `{remindersSent, pastDue, lapsed, downgradesApplied}`.
- The route is `GET` only, not `POST` as §7 names it: Vercel Cron Jobs only ever call a route with `GET` (no method is configurable), adding `Authorization: Bearer $CRON_SECRET` itself. `POST` is deferred until a manual/scripted trigger actually exists to call it — adding an unused method was speculative surface, not a real requirement of this Part.
