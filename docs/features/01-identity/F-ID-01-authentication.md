...
| Area             | auth                                                                                                                                                                          |
| Status           | planned                                                                                                                                                                       |
| Owner branch     | `feat/identity-authentication`                                                                                                                                                |
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| Password minimum length       | 10 characters                                                                                                      | `packages/domain/auth/passwordPolicy.ts`     |
...
- **OQ-3: Account-level settings route.** ARCHITECTURE §2 defines no `(account)` route group, so Security/Devices/Delete render inside each shell's settings (`/app/settings/security`, `/personal/settings`). **Default assumed: shell-local, shared components.** If the owner prefers one canonical `/account/*`, ARCHITECTURE §2 must change first.
