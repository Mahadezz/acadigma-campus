...
On the PWA, "Turn on push" requests the browser permission and registers a Web Push subscription into `device_registrations`. In the Android wrapper, the Capacitor Push plugin registers an FCM token into the same table (ARCHITECTURE §7). The UI never claims push is on when the OS permission is denied — it shows "Blocked in your browser settings" with instructions. Unsubscribing sets `revoked_at`.

Because the wrappers land after web Release 1 (DECISION-LOG D-13), v1 ships Web Push on the PWA and the same table and job serve FCM later with no schema change.
...
| Quiet hours          | **not in v1**; push respects the OS-level do-not-disturb only                                                                                                                                  | OQ-3                                                          |
...
**Part 5 — Push (Web Push) and device plumbing** · `registerPushDevice`/`revokePushDevice`, the VAPID setup, the `notification.push` job with provider error handling and token revocation, the honest permission-denied state, `push_log`. · Tests: job unit tests for each provider outcome; an e2e that registers, receives and clicks a push in a Playwright context with permissions granted; a test that a revoked token is not retried. · **Demo:** install the PWA on an Android phone, background it, and receive a tappable push that deep-links correctly.
...
- **OQ-1: push in v1.** ARCHITECTURE §7 assigns push to the native wrappers, which land after web Release 1. **Default assumed:** Web Push ships with the PWA in this feature (same table, same job), and FCM is a driver added in the wrapper phase. If Web Push is dropped, Part 5 becomes wrapper-phase work and the preference column stays, showing "Available in the app".
...
- **OQ-3: quiet hours.** Not specified. **Default assumed:** none in v1; push respects the device's do-not-disturb. A `quiet_hours` jsonb on `user_preferences` is the obvious future addition.
