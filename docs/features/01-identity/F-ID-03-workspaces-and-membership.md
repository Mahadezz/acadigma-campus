...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Area             | auth                                                                                                                                                                                                                                                |
| Status           | in-progress — Parts 1-3 built on `feat/identity-tenancy` (tenancy hardening migration + RLS, `WorkspaceContext` resolution, permission matrix + nav engine; Parts 4-8 remain)                                                                       |
| Owner branch     | `feat/identity-tenancy`                                                                                                                                                                                                                             |
...
### Deviations recorded while building Parts 1-3

- **Tables/RLS/last-owner/self-edit already existed.** `workspaces`, `school_profiles`, `workspace_members`, `custom_labels`, `workspace_member_capabilities`, the five `app.*` helpers, `app.tg_workspace_members_guard()` (last-owner invariant + self-edit ban) and `app.next_id` all shipped in `20260917010100_identity.sql` (M0 foundation), not freshly in this Part. Part 1's actual net-new work was: the `public.switch_workspace`/`public.list_my_workspaces`/`public.log_tenancy_context_rejected` RPCs (D-50) and closing a real gap — `school_profiles.workspace_id` (its own primary key) had no `app.attach_freeze_workspace()` trigger, so an owner/admin of two workspaces could re-parent a school's profile row between them despite the WITH CHECK role predicate alone appearing to guard it (D-36's degenerate-template warning, concretely realised).
- **Cookie name.** The codebase's existing cookie is `acadigma_workspace` (set in `apps/web/lib/supabase/middleware.ts` before this Part), not `acx_ws` as this doc's §4.3 names it. Kept the existing name rather than introducing a second one; middleware already mirrors it into `x-workspace-id` on every request, which collapses this doc's steps 1-2 into one for `resolveWorkspaceContext`.
  - **Update (review follow-up, 2026-09-24, D-52):** collapsing steps 1-2 this way had a real edge, now mitigated. Because middleware mirrors the cookie into the header for EVERY request regardless of whose session it is, a stale cookie left by a previous, un-signed-out session on a shared device outlived that session and was mirrored into the FIRST request of the NEXT person's new one. `signInWithPassword`, `resetPassword`'s `verifyOtp`, and `/api/auth/callback`'s `verifyOtp` now clear `WORKSPACE_COOKIE` as soon as they mint a session, and `apps/web/lib/resolve-landing-route.ts` resolves workspace context with an EMPTY header bag rather than the real request headers (the response cookie-clear cannot retroactively change the headers Next already attached to the request in flight). See D-52 and `apps/web/app/(auth)/actions.test.ts` / `apps/web/lib/resolve-landing-route.test.ts`.
- **`default_workspace_id` (§4.3 step 3) does not exist.** `user_preferences` has no such column. The implemented order is header → `profiles.last_active_workspace_id` → first active membership (personal first); adding a `user_preferences.default_workspace_id` hop is deferred to whichever Part first needs a workspace default independent of "last opened".
- **`set_config('app.workspace_id', ...)` from `resolveWorkspaceContext`, as literally described, cannot propagate:** PostgREST/`supabase-js` gives every `.from()`/`.rpc()` call its own transaction, so a GUC set in one call is invisible to the next. `resolveWorkspaceContext` stays a pure verification query; a mutation RPC that needs `app.current_workspace_id()` (e.g. inside the generic audit trigger) calls `app.set_workspace_context()` itself as its first statement, in the same transaction as its actual write. `public.switch_workspace` does not currently do this (it has no need to — its own query already carries the workspace id explicitly) but is the template for the next Part that does.
- **OQ-1 implemented as stated**: `resolveLandingRoute` is type ∧ role, role as tie-break; `packages/domain/src/workspace/resolveLanding.ts`.
- **`packages/domain/modules.ts` (the module dependency graph) was not built in this Part** — out of the Part 3 scope handed to this session (permission matrix + nav engine only). `packages/domain/src/nav/` gates nav items on a module key but does not itself compute `effectiveVisibility`; callers pass `entitledModules`/`hiddenModules` sets into `filterNav`.
- **The Semgrep/ESLint rule for "every repository export's first parameter is `WorkspaceContext`" (§9 AC18) was not added** — no `packages/db` repository functions exist yet to protect (no feature area has shipped one); revisit when the first one does.

### Review follow-ups (2026-09-24, D-52)

- **New `WorkspaceContextFailureReason`: `membership_inactive`.** §4.3 step 3's "no row → `403 WORKSPACE_NOT_MEMBER` plus the tripwire" was one case covering two different situations: a genuine forged header, and a removed or pending member replaying a stale header/cookie. `resolveWorkspaceContext` now checks — only when the active-membership lookup on the header path comes back empty — whether the caller has ANY `workspace_members` row for that workspace, in any status (RLS already allows a user to see their own row in every status, §3). A row found fails `membership_inactive` and does **not** write `tenancy.context_rejected`; no row at all keeps the original behaviour. AC6 is unaffected: `requireWorkspace()` renders the same `forbidden()` screen either way.
- **`app.tg_freeze_workspace()` gained one narrow exception.** `public.data_requests.workspace_id` is `on delete set null` (a DSAR outlives the school it names), which is itself implemented as an internal UPDATE and so was blocked by the freeze trigger this Part attached to that table. `20260924010000_tenancy_freeze_cascade_exception.sql` allows exactly the `not null → null` transition, only on `data_requests`, only when `pg_trigger_depth() > 1` (the FK cascade's own signature — a direct client statement always runs at depth 1). Every other re-parent attempt is unaffected. See `docs/architecture/DATA-MODEL.md` §7.5 and `supabase/tests/10_tenancy_cascade.sql`.
