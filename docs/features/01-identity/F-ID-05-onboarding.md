...
- **The one real gap:** the partial unique index this section and PRODUCT-DECISIONS §1.2 both promise ("enforced by a partial unique index") did not exist. Added in `20260925000100_personal_workspace_uniqueness.sql` as `workspaces_one_personal_per_creator`, a unique index on `workspaces(created_by) where type='personal'` — `created_by` rather than `owner_id` per this section's own wording, because `owner_id` can move under `app.transfer_ownership()` (a school-workspace operation) while `created_by` is the immutable "who this was made for" fact. This is defence in depth: `app.handle_new_user()` already creates at most one by construction (it runs once per `auth.users` row), so the index guards against a future bug or a direct administrative insert, not the normal signup path. Proven by `13_personal_workspace_registration.sql`'s duplicate-insert case (`23505`).
- **No separate `workspace.created` audit action was added.** `public.workspaces` has carried the generic `app.attach_audit()` trigger since M0 (`20260917010200_audit_and_files.sql`, re-affirmed by D-51's audit substrate), which already writes a `workspaces.insert` row on every insert, including the registration-time personal workspace. D-51 deliberately keeps the generic `<table>.<op>` action space separate from curated business-action names introduced by explicit `app.log_audit_event()` calls; adding a second, semantically-named `workspace.created` row for the same insert would duplicate the audit trail rather than fix a gap. Treated as a deviation from this section's exact wording, not a missing behaviour — the school-creation transaction in Part 4 faces the same choice and should stay consistent with whatever this PR's review decides.
- **DATA-MODEL.md §1.2** now documents `workspaces_one_personal_per_creator` alongside the other `workspaces` indexes.
