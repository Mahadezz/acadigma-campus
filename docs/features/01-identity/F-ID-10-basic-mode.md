...
- **A real bundle-size defect was found and fixed, the same class D-405 item 8 hit**: the first version measured 257 kB gzipped (over the 250 kB `check-bundle-budget.mjs` gate) from Radix `Tabs` plus three tab components' code all bundled eagerly. `next/dynamic(..., { ssr: false })` on Marks/Students/Print and replacing `@acadigma/ui/components/tabs` with a hand-rolled `role="tablist"` (this switcher needed none of Radix's roving-tabindex/orientation logic) brought it to 244 kB. `ssr: true` (next/dynamic's default) does **not** shrink first-load JS even when the import is code-split — `ssr: false` is required, a corrected detail for whoever next reaches for `next/dynamic` on a bundle-budget problem in this codebase.
