...
- **A real bundle-size defect was found and fixed, the same class D-405 item 8 hit**: the first version measured 257 kB gzipped (over the 250 kB `check-bundle-budget.mjs` gate) from Radix `Tabs` plus three tab components' code all bundled eagerly. `next/dynamic(..., { ssr: false })` on Marks/Students/Print and replacing `@acadigma/ui/components/tabs` with a hand-rolled `role="tablist"` brought it to 244 kB. `ssr: true` (next/dynamic's default) does **not** shrink first-load JS even when the import is code-split — `ssr: false` is required, a corrected detail for whoever next reaches for `next/dynamic` on a bundle-budget problem in this codebase. **Re-verified in review (2026-09-27, per the owner's shadcn-first rule):** swapping the hand-rolled switcher back for real `@acadigma/ui/components/tabs` (Radix) was measured on this branch after merging `origin/main` — 251 kB gzipped, 1 kB over budget, against 245 kB for the hand-rolled version (both numbers shifted slightly from the originals above because shared chunks changed on `main`). The hand-rolled switcher stays for that reason, not by default; it now also reproduces Radix `Tabs`'s own keyboard contract by hand (WAI-ARIA APG "Tabs," automatic activation: roving `tabindex`, Left/Right/Home/End), which the original version didn't have.
