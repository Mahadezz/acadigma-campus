...
- **§2 footnote ² — interim assignment source, on purpose.** `section_subjects` (F-AC-01) was still open as PR #73, not merged, when this Part shipped. `getBasicHome` uses the spec's own named stand-in: class-teacher sections (`attendance_day`'s `is_mine`) plus `exam_subjects.teacher_id` (F-AC-06's stand-in for `section_subjects.teacher_id`). It sits behind one function (`resolveMyAssignments` in `packages/db/src/repositories/basic-home.ts`), a one-line swap to `section_subjects`' `listMySections` once #73 merges (D-405 item 1).
- **A section can produce two blocks**, not one that lists several subjects — a class teacher who also teaches a subject in that same section gets a "Class teacher" block and a subject block, per §2 footnote ²'s own "plus every (section, subject) they teach" (D-405 item 2).
