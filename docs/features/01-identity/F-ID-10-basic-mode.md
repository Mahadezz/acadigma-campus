...
- **Essentials row is Settings · Language · Switch to full app · Sign out** (review batch, 2026-09-26 — originally shipped as Profile · Settings · Switch to full app) — §4.4's own listing includes Help, but it is not repeated here since `BasicShell`'s top bar already carries it on every basic screen (D-405 item 6). Sign out and the language switch replace basic mode's missing `UserMenu` (spec §6's "Basic settings" row list — Text size, Language, Switch to full app, Sign out — is the evidence both were always meant to be reachable); Profile (`/account/security`, a layout-less dead end) is dropped rather than given a shell, since no dedicated profile screen exists anywhere in the app yet; "Settings" reuses `/app/settings` unchanged. Full detail in D-405's review addendum.
...
- **A real bundle-size defect was found and fixed**, not merely worked around: `/app/home` measured 276 kB gzipped first-load JS against the CI-enforced 250 kB budget (`check-bundle-budget.mjs`) — a Server Component importing `@acadigma/ui/components/button` at all pulls the whole `radix-ui` package into that route (the same root cause D-305 item 8 and `(school)/app/reports/runs/[id]/page.tsx` already hit). Originally fixed by inlining the button classes as literal strings instead of importing `Button`/`buttonVariants` from a server file (`/app/home` measured 195 kB); the review batch below replaced the literal strings with `buttonVariants` imported from a new radix-free `packages/ui/src/components/button-variants.ts` instead, so the classes stay real rather than a copy that drifts — `/app/home` now measures 196 kB, still comfortably under the 250 kB gate. §5.5's own stricter, spec-only 150 kB target for `/app/home` is not met — only the repo's actual, CI-enforced 250 kB budget is; recorded honestly in the test report rather than silently treated as met.
...
- **Review batch (2026-09-26), full detail in D-405's addendum:** Sign out and the language switch added to the essentials row, Profile dropped; `HelpSheet`'s close control is now a full-width "ফিরে যান / Go back" button, not a 16px English-only "✕"; the phone Call button (found via this batch's own manual screenshot pass at 360px + Extra large + বাংলা) now wraps onto two lines instead of clipping past its own edge; `(school)/app/layout.tsx`'s `getSchoolProfile` request waterfall is gone (`React.cache()`, keyed on `workspaceId`); the home error state is localised with a Retry link; `t.basicMode.help.routes.home` gains the marks-entry line §8 Part 3 above already documents.

**Part 3 status/deviations (D-406, shipped 2026-09-26):**

- **A class block, the "All classes" list, and the "roll calls not taken" to-do's destination change**: a class tap now opens `/app/classes/[sectionId]` (this Part), not the roll call directly (D-405 item 4's interim). The "roll calls not taken" to-do still opens `/app/attendance` — it is not scoped to one class, so it has no single hub to open.
- **Attendance tab is the existing `RollCall` component itself**, given two new optional props (`basic`, `basicCopy`) rather than a copy or a wrapper — the plain `/app/attendance/[sectionId]` page passes neither and is byte-for-byte unchanged. §4.6's "row 'More' button instead of long-press" does not apply: F-AC-03's long-press popover (§4.1 step 3) does not exist in the shipped `AttendanceToggle` (a fully visible 5-segment control already), so there is nothing to convert.
- **§4.5's "Earlier days" (the session list) is not built.** No repository anywhere in the codebase (F-AC-03 included) lists a section's past attendance sessions; the Part's own brief was "today's roll call inline or one tap to it," which this ships. Adding a session-history read is a larger, separate piece of work for whichever Part actually needs it.
- **§8 Part 3's "F-OP-03 P6 if merged, else omit" applies to register printing on the Attendance tab, not to the Print tab.** F-OP-03 Part 6 (register/mark-sheet PDF export) is not merged, so no register-print button exists anywhere in this Part. The Print tab itself is unconditional (per this Part's builder brief) and reuses F-OP-03 Parts 3/5's already-shipped single/bulk report-card actions (D-206/D-207), which do not depend on Part 6.
- **Marks tab shows papers by "teaches" (subject teacher or class teacher of the section), not by role.** Matches `getMarkSheet`'s own definition (`marks.ts`) rather than a plain owner/admin-see-more rule — an owner/admin who is not personally the paper's or section's teacher sees the same empty state a teacher would.
- **`CLASS_HUB_TABS` (`packages/domain/src/class-hub`) holds only the four tabs this Part ships** — Routine/Print(register)/Handouts/Lesson plan/Homework are not placeholder entries; each is added by its own future Part, per this section's own closing note.
- **A real bundle-size defect was found and fixed, the same class D-405 item 8 hit**: the first version measured 257 kB gzipped (over the 250 kB `check-bundle-budget.mjs` gate) from Radix `Tabs` plus three tab components' code all bundled eagerly. `next/dynamic(..., { ssr: false })` on Marks/Students/Print brought it to 244 kB. **Ships with real `@acadigma/ui/components/tabs` (Radix), not a hand-rolled tablist** — an interim review round (2026-09-27) swapped in a hand-rolled `role="tablist"` after measuring Radix `Tabs` at 251 kB (1 kB over budget), then found that version shipped with zero keyboard handling; a lead decision (owner rule: no hand-rolled replacements for a component shadcn already has) reversed that, on the reasoning that the hand-rolled a11y gap was the bigger risk. The 1 kB (and more) was won back a different way: `RollCall`'s `ConfirmSheet` — a `Sheet` that only opens after Save is tapped, never needed for the tab's initial paint — moved behind `next/dynamic(..., { ssr: false })` too, the same treatment Marks/Students/Print already had. Final: **241 kB** on `/app/classes/[sectionId]`, **234 kB** on the plain `/app/attendance/[sectionId]` (down from 243 kB, since that route paid the same `ConfirmSheet` cost without ever rendering it). No budget-config change was needed. Full detail and every intermediate number in DECISION-LOG D-406's addenda.
- **Manual screenshots (360×800, Extra large, বাংলা) were not performed** — no live Supabase project in this sandbox (`.env.local` absent), the same limitation every prior Part's test report already recorded. Full reasoning and the intended method are in D-406's consequences.
