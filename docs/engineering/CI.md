...
2. `supabase start` — a GitHub-hosted runner is always a fresh Postgres data volume, so this also applies every `supabase/migrations/*.sql` and the seed (`supabase/config.toml`'s `[db.seed] sql_paths = ["./seed/seed.sql"]`, added by this same PR after this job's first run found the seed was silently not applying at all — the CLI's default seed path is `supabase/seed.sql`, not this repo's `supabase/seed/seed.sql`) before PostgREST/GoTrue come up in front of them.
