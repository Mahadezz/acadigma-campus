...
- `types.generated.ts` is **committed**. CI fails if it does not match the PR's migrations (`CI / db`, D-55). The failing run uploads the correct file as the `types-generated` artifact: `rm packages/db/src/types.generated.ts && gh run download <run-id> -n types-generated -D packages/db/src`, then commit it in the same PR as the migration. The `rm` matters — `gh run download` silently skips a file that already exists at the destination and prints nothing, so without it the download does nothing and the mismatch looks unfixed.
- **Migration order (D-69, D-70):** migration timestamps are real UTC time (`date -u +%Y%m%d%H%M%S`, no lane digit — `docs/plan/LANES.md`), and a new migration's timestamp must also sort after the newest migration on `main` at merge time, not just when it was written — `scripts/check-migrations-order.mjs` (`CI / db`) checks this against the merge base, separately from the append-only check. On a red order check: `git mv supabase/migrations/<old-name>.sql supabase/migrations/<suggested-name>.sql` (the suggestion in the `::error` is the current UTC time, or one second past `main`'s newest if that isn't later), update any reference to the old filename, and re-run the gate. The file has never been applied, so renaming it is safe.
