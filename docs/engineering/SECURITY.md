...
| Realtime channels re-authorise on reconnect and RLS applies to the publication, so an open subscription stops delivering                                                  | ARCHITECTURE §5 |

**Proof:** `T-ESC-04` (pgTAP: flip status to `removed`, same session, zero rows visible on every tenant table), `T-CTX-02` (stale cookie → 403), e2e `removed-member-loses-access` (teacher is removed mid-session; the next navigation lands on the no-access screen without re-login).
