...
- **Offline (D-71, F-ID-11):** whole-app, everything except generating or sending. PWA (service worker via `@serwist/next`): app shell and opened pages cached; one IndexedDB outbox per user queues every offline-capable write and replays it through the same server actions with idempotency keys, on app open, on `online` and on visibility change — no Background Sync on any platform (iOS has none). Purged on sign-out, revocation and role change. **As built (Part 1, D-308):** `apps/web/app/sw.ts` caches full page loads of `/app`, `/family`, `/personal` network-first in `acadigma-data-pages`; RSC payloads, `/api/*`, auth and `/platform` are network-only. Every user-data cache is named `acadigma-data-*`; `apps/web/lib/offline` wipes them when `GET /api/offline/session` shows a different user, workspace or role, a removed membership or no session. Generate/send controls wrap in `OnlineOnly` ("Needs internet").
