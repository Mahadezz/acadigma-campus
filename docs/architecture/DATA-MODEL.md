...
- **`sections` and `subjects` shipped with the F-AC-01 demo cut** (`20260925300203_sections_and_subjects.sql`, D-102). Cross-tenant references are impossible by construction: `academic_years`, `grade_levels` and `workspace_members` gained a `unique (id, workspace_id)` so `sections` can use composite foreign keys that include `workspace_id`. Both tables follow the T2 template (freeze, `updated_at`, generic audit with catalogue rows, `require_writable`, immutable `created_by`). Rooms, terms, `grade_level_subjects` and `section_subjects` come with later F-AC-01 Parts.
