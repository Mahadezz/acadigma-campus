...
**RLS** — SELECT `{owner,admin}` **or** `email = (select app.current_email())` **or** platform. INSERT `{owner,admin}` and `invited_by = auth.uid()` and `status='pending'`. UPDATE/DELETE `{owner,admin}` (revoke, resend). INSERT and UPDATE also require an owner when the row's `role` is `owner` (D-75, `20260926182848_security_audit_p1.sql`: the same rule `app.create_invitation` and the members guard apply; a direct write used to skip it). Accept and decline never touch these policies.
...
| `section_subjects`        | Section × subject × teacher — shipped, demo cut (D-107)                  | `section_id`, `subject_id`, `teacher_id` (all composite FKs with `workspace_id`; teacher an active owner/admin/teacher by trigger); later: `assistant_teacher_ids uuid[]`, `periods_per_week`, `is_optional_fourth`                                                                                              | ✔              | T2                                                                                                                                                                                                                                | `unique(section_id, subject_id)`; `(workspace_id, teacher_id)` — workload + "my classes"                                                                                                                                                                                                    | no          |
...
| `guardian_users`          | Links a parent account to a guardian (so to one child) — shipped (D-306) | `guardian_id`, `student_id` (denormalised; composite FK `(guardian_id, student_id, workspace_id)` → `guardians`), `user_id`, `status` (`guardian_link_status`: `invited`,`active`,`revoked`), `invited_at`, `accepted_at`, `revoked_at`                                                                          | ✔              | own rows + owner/admin and the student's current class teacher read (D-109); **no client write grant** — written only by `accept_guardian_invitation` / `revoke_guardian_link` (D-108); `invitation_id` → `workspace_invitations` | `unique(guardian_id, user_id)`; `(user_id, student_id) where status='active'` — the exact probe in `app.is_guardian_of()`; `(workspace_id, user_id) where status='revoked'` (D-109 guard probe)                                                                                             | no          |
...
- **Publishing (F-AC-06 Part 7 demo cut, `20260926023537_publish_results.sql`, D-306).** `results` gained `published bool`, `published_at`, `published_by` (→ `profiles`), `withheld_reason` (1–500 chars) and `frozen_payload jsonb` (check: published ⇒ frozen) — the report card's `ReportCardDto` built by `app.result_card_payload` at publish. `app.tg_exams_publish_results` (AFTER UPDATE OF status on `exams`) stamps and freezes every result of an exam on `→ published` (refusing `NOT_COMPUTED` and `INCOMPLETE_PRESENT` after D-304's `MARKS_INCOMPLETE` gate) with one `results.published` event, and on `published → marks_locked` clears `published` (payload kept) with one `results.unpublished` event carrying the reason. `public.publish_results(ws, exam, withhold jsonb)` (owner/admin) sets the withheld reasons, then publishes. New RLS policy `results_select_guardian`: published, not withheld, active `parent` member, `app.is_guardian_of(student_id)`; lines follow. **`guardian_users`** ships here (row above), and `guardians` gained `unique (id, student_id, workspace_id)` as its FK target; `app.is_guardian_of` now probes `guardian_users (user_id, student_id)` directly.
- **Security audit Part 1 (`20260926182848_security_audit_p1.sql`, D-75).** Index `results_workspace_exam_id_idx (workspace_id, exam_id, id)` backs `listPublishCandidates`' keyset paging past `max_rows`. `app.revoke_non_owner_owner_invitations()` (granted to nobody, run once by the migration) revoked pending owner invitations whose inviter was not an active owner. The profiles guard, `staff_documents` file FK and invitation policies are described in their own sections.
...
- **Submit, lock and the entry window (F-AC-06 Part 4 demo cut, `20260926063741_marks_submit_lock.sql`, D-307).** `exam_subjects` gained `entry_opens_on date`, `entry_closes_on date` (check: close ≥ open) and `status_reason text` (1–500 chars, the unlock reason; cleared by the next forward step). `marks` gained `edited_after_window bool` (an owner/admin last wrote it outside the window, with a reason). `app.marks_entry_window(exam_subjects)` gives the effective window (opens = the date set or `exam_date`; closes = the date set or the later of opens and `exams.ends_on`, + 7; no date = no window; existing `marks_entry` papers backfilled to today + 7), enforced by `public.save_marks` (`OUTSIDE_ENTRY_WINDOW` for teachers, `late_reason` required from owner/admin). `public.submit_exam_subject(ws, paper, confirm)` (paper's teacher or owner/admin), `public.lock_exam_subject(ws, paper)` and `public.unlock_exam_subject(ws, paper, reason)` (owner/admin) are SECURITY DEFINER; unlocking on a `marks_locked` exam moves the exam back to `marks_entry`, which clears its results. `app.tg_exam_papers_lock` allows `locked → submitted` only inside `unlock_exam_subject` (transaction-local `app.unlock_via_rpc`) and with a `status_reason`. `edited_after_window` is sticky. Audit: `marks.submitted`, `marks.locked`, `marks.unlocked`.
- **`section_subjects` shipped as the F-AC-01 Part 5 demo cut** (`20260926021923_section_subjects.sql`, D-107): `section_id`, `subject_id`, `teacher_id`, with `unique (section_id, subject_id)` and `unique (id, workspace_id)` for later composite FKs. The FKs to `sections`, `subjects` and `workspace_members` include `workspace_id`; `teacher_id` is `on delete set null`. `app.tg_section_subjects_teacher_eligible` requires an active owner/admin/teacher (`MEMBER_NOT_ELIGIBLE`), and `app.tg_members_release_subject_teacher` nulls `teacher_id` when a member stops being one. The table uses the T2 template (freeze, `updated_at`, generic audit with catalogue rows, `require_writable`, immutable `created_by`) **with** a DELETE policy/grant for owner/admin, because nothing references a row yet. **`public.set_section_subjects(workspace_id, section_id, jsonb)`** (SECURITY INVOKER) replaces a live section's subject list. **`public.create_exam`** now sets each paper's `exam_subjects.teacher_id` from the matching row, left join, null when there is none. `set_section_subjects` raises `SUBJECT_ARCHIVED` for an archived subject. `app.tg_require_writable` lets through, in read-only mode, an UPDATE that only clears `sections.class_teacher_id` or `section_subjects.teacher_id`, so removing a member always works (D-300, review of PR #73).
- **Guardian linking (F-AC-02 Part 4 demo cut, `20260926065723_guardian_linking.sql`, D-108).** `workspace_invitations` gained `guardian_id` + `student_id` (composite FK → `guardians (id, student_id, workspace_id)`; checks: both or neither, and `role = 'parent'` ⇔ a guardian is named; partial index on `guardian_id` where pending); `guardian_users` gained `invitation_id` (→ `workspace_invitations`, set null). Four SECURITY DEFINER functions granted to `authenticated`: `invite_guardian(ws, guardian)` (owner/admin; 30-day single-use token, only its hash stored; replaces the guardian's pending link; `RATE_LIMITED` past 60 guardian invitations an hour per school), `guardian_invitation_preview(token)`, `accept_guardian_invitation(token)` (a parent membership and an active link to the invitation's own student; `MEMBERSHIP_CONFLICT` for anyone in the school in another role) and `revoke_guardian_link(ws, link)`. New policy `students_select_guardian`: a parent member reads their linked, non-deleted children's `students` rows. `revoke_guardian_link` also removes the parent's membership with their last link; trigger `workspace_members_revoke_guardian_links` (AFTER UPDATE OF status → `removed`) revokes the person's active links whatever removed them, and `app.tg_require_writable` allows a revoke-only `guardian_users` update in a read-only school. `app.tg_workspace_members_guard` lets a removed parent reactivate their own membership only inside `accept_guardian_invitation` (a guardian invitation they accepted in the same transaction; no other column may change). `files_select_member` (workspace visibility) and `usage_counters_select` now exclude parents. `app.accept_invitation` / `app.decline_invitation` ignore guardian invitations.
- **Guardian follow-ups (`20260926180341_guardian_followups.sql`, D-109).** `students_select_guardian`, `results_select_guardian` and `public.family_results` require an active membership of **any** role plus `app.is_guardian_of` (staff who are also parents), instead of `role = 'parent'`. `guardian_users_select` adds `app.can_read_student_private` (owner/admin, the current class teacher) for students that are not soft-deleted; partial index `guardian_users_revoked_probe_idx (workspace_id, user_id) where status = 'revoked'` serves the members guard's probe. `invite_guardian` / `revoke_guardian_link` accept owner/admin or the class teacher of the student's current section, 60 invitations an hour per inviter; `accept_guardian_invitation` refuses the inviter (`FORBIDDEN`). `accept_guardian_invitation` keeps an active member of any role unchanged (link only); `MEMBERSHIP_CONFLICT` only for a pending or removed staff membership. `app.tg_workspace_members_guard` accepts one more transition: a `parent` membership `active → removed`, nothing else changed, only from inside a SECURITY DEFINER function (`current_user` not `authenticated`) with a link of that person revoked at `now()` and none left — so a class teacher's `revoke_guardian_link` removes a pure parent with their last link.
