...
**Indexes** — `unique (slug)`. `unique (invite_code) where invite_code is not null`: join-by-code probes this on every attempt and two schools must never share a code. `(owner_id)`: FK column, "workspaces I own", ownership transfer. `(plan_id) where not null`: FK column, /platform plan breakdown. `(trial_ends_at) where trial_ends_at is not null and status='active'`: the nightly trial-expiry job scans only live trials.
**RLS** — SELECT `app.member_role(id) is not null` or platform. INSERT `owner_id = created_by = (select auth.uid())`. UPDATE `{owner,admin}`, plus a platform policy. DELETE platform only — tenants archive.
**Triggers** — `tg_workspace_defaults` (BEFORE INSERT: invite code for schools, none for personal); `tg_workspace_bootstrap` (AFTER INSERT: owner membership + `school_profiles` row); `tg_workspace_billing_bootstrap` (AFTER INSERT: Free for personal, 14-day Pro trial + `subscriptions` row for schools); `tg_workspaces_guard` (BEFORE UPDATE: `type`, `owner_id`, `invite_code` immutable for everyone; `plan_id`, `trial_ends_at`, `status` platform-only); `updated_at`; audit.
...
| ------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `attendance_policy` | `SchoolAttendancePolicy` | `mode` = `daily` or `period` (`daily`), `cutoff` (`"09:15"`), `late_counts_present` (true), `half_day_counts_present` (true), `min_attendance_bp` (7500), `block_exam_on_shortfall` (false) |
...
| `grade_levels`            | Class 1…12 / Play Group                            | `name`, `level_number`, `group` (`early`,`primary`,`secondary`,`higher`)                                                                                                                         | ✔              | T2                             | `unique(workspace_id, level_number)`                                                                                                                                                       | no          |
| `subjects`                | Subject catalog per school                         | `name`, `code`, `category`, `subject_kind`                                                                                                                                                       | ✔              | T2                             | `unique(workspace_id, lower(name))`                                                                                                                                                        | no          |
| `sections`                | The homeroom — "Class 6 – A"                       | `grade_level_id`, `academic_year_id`, `name`, `class_teacher_id`, `room`, `capacity`, `student_count` (trigger)                                                                                  | ✔              | T2                             | `unique(workspace_id, academic_year_id, grade_level_id, name)`; `(workspace_id, class_teacher_id)` — "my sections"                                                                         | no          |
...
| `students`                | The child record                                   | ~40 columns in blocks: identity, ID doc, addresses, admission (`student_code` from `app.next_id`), previous school, health, interests, status, `needs_attention`, `attention_reason`, `is_draft` | ✔              | T4                             | `unique(workspace_id, student_code)`; `(workspace_id, status) where deleted_at is null`; `(workspace_id) include (full_name)` for search; trigram index on `full_name` for the search box  | **yes**     |
| `guardians`               | Parent/guardian **data**, not a user               | `student_id`, `relation`, `full_name`, `phone`, `email`, `occupation`, `is_primary`, `can_pickup`, ID-doc fields                                                                                 | ✔              | T4                             | `(workspace_id, student_id)`; `unique(student_id) where is_primary`                                                                                                                        | no          |
...
| `enrollments`             | Student ↔ section for one year                     | `student_id`, `section_id`, `academic_year_id`, `roll_number`, `status`, `enrolled_on`, `left_on`                                                                                                | ✔              | T2                             | `unique(student_id, academic_year_id)` — one section per student per year (2.3); `unique(section_id, roll_number, academic_year_id)`; `(section_id) where status='active'` — the roll call | no          |
...
| `attendance_sessions`     | One roll call (section, date, optional period)     | `section_id`, `date`, `period_id`, `taken_by`, `taken_at`, `status`, `locked_at`                                                                                                                 | ✔              | T1                             | `unique(section_id, date, coalesce(period_id, uuid_nil))`; `(workspace_id, date desc)` — the register                                                                                      | no          |
| `attendance_records`      | One student's status in a session                  | `session_id`, `student_id`, `status` (`present`,`absent`,`late`,`excused`,`half_day`), `note`, `marked_by`                                                                                       | ✔              | T4                             | `unique(session_id, student_id)`; `(workspace_id, student_id, date)` via a denormalised `date` column — every attendance-% query is per student over a date range                          | no          |
...
22 tables: hiring, cover, print, messaging, and the personal workspace.
...
| `staff_records`       | Employment record created on hire — **no pay columns** | `member_id`, `application_id`, `employee_code`, `designation`, `joined_on`, `contract_type`, `contract_signed`, `id_issued`, `orientation_done`, `system_access`, `left_on`                                                                   | ✔                      | T2                                  | `unique(member_id)`; `(workspace_id, joined_on desc)`                                                                                                                                                                                               | no          |
| `staff_compensation`  | Period-versioned pay, readable by `{owner,admin}` only | `user_id`, `hourly_rate_paisa`, `monthly_salary_paisa`, `currency`, `valid_from date`, `valid_to date`, `note`                                                                                                                                | ✔                      | read/write `{owner,admin}` only     | `exclude using gist (workspace_id =, user_id =, daterange(valid_from, coalesce(valid_to,'infinity'), '[]') &&)` — one rate in effect at a time, enforced rather than assumed; `(workspace_id, user_id, valid_from desc)` — the point-in-time lookup | no          |
...
So compensation lives in `staff_compensation`, a separate table whose policies are `{owner,admin}` and nothing else, versioned by period so a raise is a new row rather than an overwrite — which is what makes a historical payroll snapshot reconstructible at all.

The cover-teacher payroll calculation needs the rate but must not be able to _see_ it. It calls `app.staff_hourly_rate(workspace_id, user_id, on_date)`, a `SECURITY DEFINER` function that returns one number for one date. The caller gets the figure the calculation needs and no access to the row it came from. `payroll_impact_logs` then snapshots that number, so re-running a month later against a changed rate cannot rewrite history.
...
`id bigint identity` · `workspace_id uuid` · `actor_id uuid` · `action text` (`students.update`) · `table_name text` (`public.students`) · `row_id uuid` · `before jsonb` · `after jsonb` · `correlation_id uuid` · `ip inet` · `user_agent text` · `created_at timestamptz`.
...
**Indexes** — `(workspace_id, created_at desc)`: the viewer pages one workspace newest-first, and it is also the policy column, so policy and query share one index. `(table_name, row_id, created_at desc)`: _"who changed this child's medical record, and when"_ — the question the prototype could not answer. `(actor_id, created_at desc) where not null`: incident response. `(correlation_id) where not null`: joins a pino log line to its database effects.
...
**RLS** — SELECT: workspace **owners** and platform staff. Admins are excluded on purpose: the log has to record what an admin did without that admin curating it. No write policy at all.

**Exactly two writers, both inside the caller's transaction:**

1. `app.tg_audit()` — the generic row-change trigger (§10).
2. `app.log_audit_event(action, workspace_id, table_name, row_id, before, after, ip, user_agent, correlation_id)` — for auditable things that are **not** row changes: a login, an invitation redeemed, a signed URL issued for an ID document, platform staff opening a support grant, `access_mode` flipping to read-only.
...
**Retention** — indefinite. Monthly range partitioning is the planned next step once the table passes ~50 M rows.
...
```sql
-- The hourly rate in effect on a date. Exists so that pay never becomes a
-- column a colleague can read: staff_compensation is owner/admin-only, and
-- the payroll calculation gets one number instead of access to the row.
