...
- **Submit, lock and the entry window (F-AC-06 Part 4 demo cut, `20260926030927_marks_submit_lock.sql`, D-307).** `exam_subjects` gained `entry_opens_on date`, `entry_closes_on date` (check: close ≥ open) and `status_reason text` (1–500 chars, the unlock reason; cleared by the next forward step). `marks` gained `edited_after_window bool` (an owner/admin last wrote it outside the window, with a reason). `app.marks_entry_window(exam_subjects)` gives the effective window (opens = the date set or `exam_date`; closes = the date set or opens + 7; no date = no window), enforced by `public.save_marks` (`OUTSIDE_ENTRY_WINDOW` for teachers, `late_reason` required from owner/admin). `public.submit_exam_subject(ws, paper, confirm)` (paper's teacher or owner/admin), `public.lock_exam_subject(ws, paper)` and `public.unlock_exam_subject(ws, paper, reason)` (owner/admin) are SECURITY DEFINER; unlocking on a `marks_locked` exam moves the exam back to `marks_entry`, which clears its results. `app.tg_exam_papers_lock` allows `locked → submitted` only with a `status_reason`. Audit: `marks.submitted`, `marks.locked`, `marks.unlocked`.
