...
1. `app.tg_audit()` — the generic row-change trigger (§10), attached to every table in §10's `v_tables` list. Writes `severity` from the catalogue (default `info`), `actor_kind` (`system` when `auth.uid()` is null, else `user`), and `changed_fields` (computed from the raw before/after key set **before** any redaction, so a nulled or dropped field's NAME still appears there — except a secret-deny-listed column, which is omitted from `changed_fields` too). A universal secret deny-list (`k ~* '(token|secret|password|account_number)'`) drops a matching column's name and value regardless of what a table's explicit `p_redact` list says; a separate `p_freetext` list (new third argument to `app.attach_audit()`) nulls a column's value while keeping its name in `changed_fields` (diary bodies, notes — none of the tables shipped so far use it, but the mechanism is pgTAP-tested against a scratch table).
...
**Retention** — 7 years, rolling (§7.6; PDPA rulings, DECISION-LOG). `app.purge_expired_audit_events()` is the one path that may `DELETE` from this table — it requires both a privileged context and the `app.retention_purge` transaction flag, which only it sets — scheduled monthly via `cron.schedule('audit-events-purge', …)`, guarded the same way.
