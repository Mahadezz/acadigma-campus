...
So compensation lives in `staff_compensation`, a separate table whose policies are `{owner,admin}` (plus the record's own person, read-only) and nothing else, versioned by period so a raise is a new row rather than an overwrite — which is what makes a historical payroll snapshot reconstructible at all. The only way anyone else ever learns a rate is `app.staff_hourly_rate(user_id, on_date)` (§11), which returns one number for one date and nothing else (D-63).

`staff_records` also carries `nid_number` (masked in the UI to the last 4 digits for everyone but owner/admin, and dropped entirely from every audit row by `app.audit_secret_pattern()`) and a handful of other record-visible-only columns (`personal_phone`, `emergency_contact`, `address`, `blood_group`, `date_of_birth`, `gender`, `qualifications`, `notes`) that a member may edit on their own row; the _employment_ columns (`employment_type`, `designation_label_id`, `department`, `employment_status`, `joined_on`, `left_on`, `staff_code`, and every identifier/provenance column) are admin-only even on your own record, enforced by `app.tg_staff_records_self_update_guard()` since a single RLS policy cannot split a table's columns by writer (F-OP-06 §2 footnote 1).
...
The cover-teacher payroll calculation needs the rate but must not be able to _see_ it. It calls `app.staff_hourly_rate(user_id, on_date)`, a `SECURITY DEFINER` function that returns one number for one date and checks the CALLER's own membership internally rather than trusting a passed-in `workspace_id` (D-63 — a 2-argument, self-checking signature, not the 3-argument sketch this document previously showed in §11). The caller gets the figure the calculation needs and no access to the row it came from. `payroll_impact_logs` then snapshots that number, so re-running a month later against a changed rate cannot rewrite history.
...
```sql
-- The hourly rate in effect on a date (F-OP-06 §3.2/§5.8, D-63). Exists so
