...
| `main` merges  | Merged `origin/main` twice while this PR was open: once after PR #41 (D-400 dashboard) and PR #49 (D-402 audit sentences) landed (resolved `dashboard/page.tsx` — took #41's; `(school)/app/layout.tsx` — kept both, took #41's `shell.signedInAs`/`shell.roles`; `DECISION-LOG.md`/`docs/README.md` — append-only, kept both), and again after PR #47 (F-AC-01) landed (docs/README.md only) |
