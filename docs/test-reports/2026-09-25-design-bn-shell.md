...
**What this Part is.** Before this Part, picking বাংলা on the sign-in screen only translated the auth screens (`apps/web/lib/i18n.ts`'s docblock said so explicitly) — the moment a user signed in, the whole school shell (sidebar, bottom nav, top bar, dashboard, settings) reverted to English regardless of the cookie. Separately, the audit viewer read language from `profiles.locale` directly and never looked at the cookie at all — two disconnected mechanisms in the same signed-in app, and the signed-in shell had no sign-out control anywhere. This Part is the demo-cut slice of F-ID-02 Part 4: the `acadigma_locale` cookie now reaches the whole signed-in app, `getLocale()` is the one resolver (cookie, then `profiles.locale`, then `en`) both paths go through — memoised per request with React's `cache()` so its several call sites share one lookup — and the `UserMenu`'s switch persists to `profiles.locale` too, so it follows a signed-in user across devices. The same menu now also carries the shell's first sign-out control, reusing the existing `signOut` server action. It is **not** the full Part 4 — no `user_preferences` table, no theme/palette/density, no Settings → Language screen, no new migration (`profiles.locale` already existed on `main`). See D-401 and F-ID-02 §11 for the full scope decision.
...
| CI run         | [run 36134892511](https://github.com/Mahadezz/acadigma-campus/actions/runs/36134892511) — all jobs pass (`db`/`guard` skip per OQ-27, as on every PR; Vercel's "fail" is the known build-rate-limit non-required check, D-70)                                                                                                                                                                                                                                                                                                   |
...
| Migration head | none — no migration in this Part (`main`'s own migrations from other lanes came in via the merges below)                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `main` merges  | Merged `origin/main` three times while this PR was open: (1) after PR #41 (D-400 dashboard), #45 (auth throttle), #46 (grade scales) and #49 (D-402 audit sentences) landed (resolved `dashboard/page.tsx` — took #41's; `(school)/app/layout.tsx` — kept both, took #41's `shell.signedInAs`/`shell.roles`; `DECISION-LOG.md`/`docs/README.md` — append-only, kept both); (2) after PR #47 (F-AC-01) landed (docs/README.md only); (3) for this review round, after PR #48 (F-AC-06 Part 2 exams) landed (docs/README.md only) |
...
| **Total (all projects)** | **1133**        | **1133** | **0**  | **0**   | ~30s     |

**Delta vs `main`:** +7 test files / +26 tests are this Part's own new suites (see below); the rest of the delta vs. the pre-merge count is every other lane's PRs that merged into `main` while this PR was open (BUILDER-BRIEF: "another design PR (#41) is in review — if it merges while you work, merge `origin/main` and keep both").

### New tests this Part adds

- `apps/web/lib/i18n.test.ts` — `getLocale`/`getMessages` resolution precedence: cookie first, then the signed-in user's `profiles.locale`, then `en`; never throws on a tampered cookie, an unknown `profiles.locale` value, a signed-out caller, or a Supabase error. Plus (review follow-up) a memoisation test: two concurrent `getLocale()` calls hit `auth.getUser()`/the `profiles` loader exactly once.
...
- `apps/web/app/(shared)/workspace/user-menu.test.tsx` (review follow-up) — the destructive "Sign out" item calls the existing `signOut` action exactly once, and the language `RadioGroup` still works from the same menu.
...
- `getLocale()` is memoised per request: two calls (e.g. root layout + `SchoolLayout` + `getReaderLanguage()` in the same real request) share one `auth.getUser()`/`profiles` lookup, not one each. React's real `cache()` is a no-op outside an actual Server Component render (verified directly — a bare `cache()`-wrapped function called twice from plain Node runs twice), so the test mocks `"react"`'s `cache` with a small per-test Map-based stand-in that models what a real request does.
...
- `UserMenu`'s "Sign out" item is reachable and wired: selecting it calls `signOut()` exactly once. (Radix's `DropdownMenuTrigger` opens on `pointerdown`, not `click` — a bare `.click()` left the menu closed in jsdom on the first attempt; the test fires `pointerdown`/`pointerup`/`click`.)
...
| ---------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bn-locale-shell.spec.ts` — switch to বাংলা from the user menu, verify shell + dashboard + settings, switch back | skipped (`E2E_LIVE_SUPABASE` unset) | same       | CI's `e2e` job (PR #51, [run 36134892511](https://github.com/Mahadezz/acadigma-campus/actions/runs/36134892511)) passed with this spec skipped — same as every other seeded-account journey (OQ-27). Runs for real once the live Supabase project is migrated + seeded. |
...
**Sign out (review follow-up), separate click-through run:** signed in once, opened the `UserMenu`, clicked "Log out" — redirected to `/login`. Re-requested `/app/dashboard` in the same browser context afterwards: redirected to `/login?next=%2Fapp%2Fdashboard`, proving the session was actually revoked server-side, not just a client-side navigation away from the shell. The audit trail (visible in the dashboard's "Recent activity" card in an earlier screenshot) recorded "Demo Owner সাইন আউট করেছেন" (`account.logout`, the reused action's existing audit call). Also confirmed the বাংলা label ("লগআউট") and the destructive red styling render correctly — screenshot below.
...
| User menu + sign out (bn) | —                                                   | `bn-shell-usermenu-signout-bn.png`   |
...
The sign-out addition calls the pre-existing `signOut` server action (`(auth)/actions.ts`) rather than adding a new one — no new attack surface. Manually verified it actually revokes the session server-side (§5), not just a client-side redirect.
...
| 5   | **Follow-up, recorded per review:** `bn-locale-shell.spec.ts` persists `profiles.locale` on the shared seeded `owner@acadigma.test` account — every other seeded-account journey in this repo assumes that account renders in English. Mitigated with a `test.afterEach` that always switches back to English, but a test in a different spec file running concurrently in another CI worker during the (short) window this suite is mid-বাংলা is not fully race-proof. Sign-out was deliberately kept unit-tested only (not added to this e2e spec) so it does not widen the same race with a second shared-state mutation. | low      | yes — same shared-seed-account tradeoff `school-settings.spec.ts` already accepts for its own mutation; a dedicated locale-test seed account removes the race entirely and is the real fix, not built here | OQ-27            |
...
| Unit tests + coverage thresholds             | ☑ (1133/1133 passing at merge time; count keeps moving as sibling lanes merge into `main`) |
