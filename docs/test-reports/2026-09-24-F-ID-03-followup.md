...
| §4.3 failure case (new, this Part)   | Well-formed header, removed/pending row → 403 `membership_inactive`, tripwire fires      | `workspace-context.test.ts` "removed/pending member vs. a genuine forger" describe block                                                                                                                                            |
...
## 10. Addendum — review amendment and CI results, 2026-09-24

**CI on the original head `02b964c`:** all 13 required checks green. The `db` job ran 9 pgTAP files, 198 tests, **PASS**, including `10_tenancy_cascade.sql`. That proves the `pg_trigger_depth() > 1` cascade exception against real Postgres, so known issues #1 and #2 in §8 are **resolved**, and the §9 row "☐ executed" is now ☑ for that head.

**Opus review of PR #12:** MERGE-WITH-FOLLOWUPS. One major finding, applied in this PR before merge: skipping the tripwire for removed/pending callers let anyone holding an invite code (who becomes `pending`) probe a school with no audit trace. Now the tripwire always fires, and `public.log_tenancy_context_rejected` records `membership_status` and `severity` server-side (`20260924020000_tenancy_tripwire_membership_status.sql`, D-52 amended).

| Test                                                                                                                                                        | Where                                                         | Result                                                              |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------- |
| Removed and pending members get 403 `membership_inactive` **and** one tripwire call                                                                         | `packages/db/src/workspace-context.test.ts`                   | PASS locally (26/26 in the file)                                    |
| Pending, removed and stranger each write one row, classified `pending`/`removed`/`none`, severity `inactive`/`inactive`/`forgery`; the owner sees all three | `supabase/tests/11_tenancy_tripwire_status.sql` (new, plan 9) | CI `db` job — see the PR checks for the head carrying this addendum |

Still open (non-blocking): the shared-phone flow has unit-level coverage only; a live two-sign-in e2e journey waits on OQ-26/OQ-27.
