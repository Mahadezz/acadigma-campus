...
| L5  | LOW              | `files_insert` lets teacher/staff create `visibility = 'public'` rows, which `files_select_public` shows to `anon` (metadata only; storage is not wired).                                                                                                                                                                                                         | policy text                                                                                                            | **Handed on**: must be fixed (with client-set `bucket`/`path` on `files_insert`) before storage or the files route lands                                            |
...
| pgTAP, this branch                                    | 48 files, 1,380 tests, PASS after merging #85 (new: `23_security_audit_identity.sql` 18, `24_parent_invoker_reads.sql` 6) |
| `23_…` before the migration                           | 8 of the first 12 assertions red (A2–A4, A6, B9–B12); C13–C14 red with the old policies restored                          |
| Integration (`*.integration.test.ts`, real PostgREST) | before the fix: 1 failed (1,000 ≠ 1,005), 7 passed; after (keyset, #85 merged): 5 files, 13 tests passed                  |
...
### Follow-ups from review (not code in this PR)

- An `auth.users` → `profiles` email/phone sync trigger is required before any email- or phone-change feature ships.
- Invitation matching by email relies on `enable_confirmations = true`; the owner is confirming the hosted setting.
- `PAGE` in `listPublishCandidates` (1000) must equal the hosted `max_rows`; the owner is confirming.
- L5 before storage or the files route.
- A one-off check that no existing `staff_documents` row points at a colleague's file.
- Lock ceiling (lead decision): the plain unique index and inline-validated FK are fine on near-empty tables; from the first real school, hot-table changes use `NOT VALID` + `VALIDATE` and a separate `CONCURRENTLY` step.
