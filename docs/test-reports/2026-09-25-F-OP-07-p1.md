...
Local run, `pnpm test` on Windows: **77 files, 881 tests, all passed** (whole repo, after merging origin/main with #32). The new suites:

| Suite                                                        | Tests | Result |
| ------------------------------------------------------------ | ----- | ------ |
| `packages/domain/src/settings/header.test.ts`                | 5     | pass   |
| `packages/contracts/src/settings-profile.test.ts`            | 6     | pass   |
...
- A save with a stale `updated_at` returns `conflict`, and the earlier save's value survives.
...
Not taken. Every screen needs a signed-in school member, and there is no seeded non-production project to sign in to (same OQ-27 blocker).
