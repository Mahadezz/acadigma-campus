...
| —    | A new tenant write action without `requireWritable` fails CI.                                      | `scripts/check-require-writable.mjs` (proven locally by removing the call: exit 1)     |
...
`supabase/tests/50_require_writable.sql`, 14 assertions: the invariant (A); owner UPDATE/INSERT/DELETE and a workspace rename refused with `42501 PLAN_READ_ONLY`, reads still return rows, a privileged write still lands, the refused rename changed nothing (B); after reactivation the same UPDATE/INSERT/DELETE succeed and land (C). Result: see the PR's `db` job.
...
| 2   | Leaving a read-only school and accepting an invitation to one are refused (`workspace_members`/`workspace_invitations` are guarded). | medium   | yes — owner question in D-300               |
