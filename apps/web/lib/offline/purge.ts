...
export function decidePurge(
  ...
  // No active membership (removed / suspended) never keeps a cache, and an
  // unknown past (`stored === null`: first check after a sign-in) is treated
  // as a different user — the phone may be shared (§5.7).
  return { purge: !same || current.workspaceId === null, next: current }
}
