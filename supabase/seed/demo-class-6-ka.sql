...
-- produces. Phone numbers are fictional (+8801700000001 … 40).
--
-- Idempotent: the idempotency keys are derived from the workspace id, so a
-- second run replays the first and admits nobody new.
--
-- Usage (psql, or the Supabase SQL editor with the first line edited):
...
begin
  if v_ws is null then
    raise exception 'set demo.workspace_id first';
  end if;
  ...
  for i in 1..40 loop
    perform public.admit_student(v_ws, jsonb_build_object(
      ...
      'guardian', jsonb_build_object(
        ...
        'phone', '+88017000000' || lpad(i::text, 2, '0'))));
  end loop;
