...
--      insert through the policy itself, which (as of D-59,
--      20260925000200_billing_bootstrap_vs_workspace_guard.sql) now succeeds
--      end to end — see 16_billing_bootstrap_guard.sql for the full proof;
--      this file keeps only the narrower "the policy itself passes it"
--      claim it originally made, from when a separate, pre-existing bug in
--      app.tg_workspace_billing_bootstrap()'s interaction with
--      app.tg_workspaces_guard() (tracked as a known issue, now fixed) made
--      the full insert fail later in the bootstrap chain.
-- =====================================================================
begin;
select plan(18);
