...
| `15_personal_workspace_registration.sql` | F-ID-05 Part 1: `app.handle_new_user()` + `app.tg_workspace_bootstrap()` create exactly one personal workspace and one active owner membership per signup, atomically (a forced failure in the LAST function of the chain still rolls back the whole `auth.users` insert, proving a real partial-write scenario unwinds, not just a first-statement failure) — `20260925000100_personal_workspace_uniqueness.sql`. Also closes an Opus-review-found gap: `workspaces_one_personal_per_creator` alone did not stop an authenticated client from `PATCH`ing `created_by` to `null` then `POST`ing a fresh `type='personal'` row; `app.tg_workspaces_guard()` now refuses any `created_by` change (`42501`) and the `workspaces_insert` policy now refuses any direct client insert of a personal workspace at all (`42501`, RLS), while a school-type insert — the one legitimate client path — still succeeds.                                                                                                                                                               |
