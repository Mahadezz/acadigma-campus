...
| `12_function_grants_invariant.sql` | D-54: for every `SECURITY DEFINER` function in `public`/`app`, `anon` and `authenticated` EXECUTE is allowed only on an explicit allowlist — never by silently inheriting Supabase's platform default; `log_auth_event_service` is `service_role`-only; and the default privilege itself, for role `postgres`/schema `public`/functions, grants EXECUTE to `service_role` but neither `anon` nor `authenticated` after migrations. Depends on `supabase/ci/bootstrap.sql` reproducing Supabase's default grant, so this file is false in CI exactly when it would be false on the platform. |
