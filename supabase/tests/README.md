...
| `14_rls_known_gaps.sql`            | D-56, `KNOWN_GAPS` shrink to empty: isolation + escalation for the six tables that already had RLS, SELECT-only (or self-scoped) grants and no unrestricted write policy, but no pgTAP case yet — `consent_records`, `legal_acceptances`, `email_log`, `file_access_log`, `subscription_events`, `notifications`. Per table: a workspace-A member never sees workspace B; `anon` holds no `SELECT` privilege at all; a non-owner/admin member sees only what the policy allows; the row's own user sees it where the policy has an own-row branch; platform admin sees every workspace where the policy grants that; and every write verb the policy does not explicitly allow is refused with `42501`.                                                                                                    |
