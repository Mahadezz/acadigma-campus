...
- 960e6fb: Fix `/app/classes` returning the generic error for every owner: `workspace_members` has four foreign keys to `profiles`, so `getClassesOverview` and `listClassTeacherOptions` now hint the intended one (`profiles!workspace_members_user_id_fkey`) instead of leaving PostgREST's embed ambiguous.
...
- 773ec87: Generated types include `public.pre_request()`, the PostgREST pre-request hook wrapper added by the D-65 hotfix (anonymous API calls were failing with 42501 on the live project).
