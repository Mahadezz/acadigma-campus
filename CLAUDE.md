...
- Every tenant write action calls `requireWritable(ctx, client)` after the policy check and returns `err(planReadOnlyApiError(w.error))`; every new `workspace_id` table calls `select app.attach_require_writable('public.<t>');` (D-300). `scripts/check-require-writable.mjs` and `supabase/tests/50_require_writable.sql` enforce both.
